...
header("content-disposition: attachment;filename=Plan_analytique.xls"); }
...
if ((isset($_POST["MM_form"])) && ($_POST["MM_form"] == "form0"))
{
  if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "MM_insert"))
  {
    ...
    $extensions_autorisees=array('xls','xlsx'); //Extensions autorisÃ©es ,'csv'
    $url_site='./attachment/'; //Adresse oÃ¹ se trouve le fichier upload.
    ...
    if(in_array($ext,$extensions_autorisees))
    {
      ...
      elseif(isset($_FILES['fichier']) && $_FILES['fichier']['error'] == 0)
      {
        ...
        $query_sup_import_annee = sprintf("DELETE from ".$database_connect_prefix."activite_projet WHERE ".(($niveau1==-1)?"":" niveau=".GetSQLValueString(intval($_GET["niveau"])+1, "int")." and ")."projet=%s",
        ...
        for ($row = 5; $row <= $highestRow; $row++)
        {
            ...
            if(!empty($code) && $code!='Code')
            {
              ...
              if($niveau1==-1){
                ...
                $query_liste_activite_1 = "SELECT code FROM ".$database_connect_prefix."activite_projet WHERE $where and projet='".$_SESSION["clp_projet"]."' ";
                try{
                    $liste_activite_1 = $pdar_connexion->prepare($query_liste_activite_1);
                    $liste_activite_1->execute();
                    $row_liste_activite_1 = $liste_activite_1 ->fetch();
                    $totalRows_liste_activite_1 = $liste_activite_1->rowCount();
                }catch(Exception $e){ /*die(mysql_error_show_message($e));*/ }
                $p = $row_liste_activite_1['code'];
                } else $p = 0;
              $insertSQL = sprintf("INSERT INTO ".$database_connect_prefix."activite_projet (intitule,niveau,code,parent,code_relai,structure,projet, id_personnel) VALUES (%s, %s, %s, %s, %s, %s, %s, '$personnel')",
                                   GetSQLValueString(trim(addslashes($rowData[0][4])), "text"),
                                   GetSQLValueString($n, "int"),
            					   GetSQLValueString($code, "text"),
                                   GetSQLValueString($p, "text"),
                                   GetSQLValueString(trim($rowData[0][7]), "text"),
              ...
              //echo $n." - ".$code." - ".trim($rowData[0][7])." - <font size='1'>".$insertSQL."</font><br />";
            }
          }
...
if ((isset($_POST["id_val"]) && !empty($_POST["id_val"]))) {
      $id = "";
      foreach($_POST["id_val"] as $a) $id .= GetSQLValueString($a, "text").",";
      $id = substr($id, 0, strlen($id)-1);
      $insertSQL = sprintf("DELETE from ".$database_connect_prefix."activite_projet WHERE id in ($id)");
      try{
            $Result1 = $pdar_connexion->prepare($insertSQL);
            $Result1->execute();
      }catch(Exception $e){ die(mysql_error_show_message($e)); }
...
if ((isset($_GET["id_sup"]) && !empty($_GET["id_sup"]))) {
      $id = ($_GET["id_sup"]);
      $insertSQL = sprintf("DELETE from ".$database_connect_prefix."activite_projet WHERE id=%s",
...
if ((isset($_POST["MM_form"])) && ($_POST["MM_form"] == "form1"))
{
  if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "MM_insert")) {
   $personnel=$_SESSION['clp_id'];
    $query_liste_actpa = sprintf("SELECT code FROM ".$database_connect_prefix."activite_projet WHERE projet=%s and code=%s",
    GetSQLValueString($_SESSION['clp_projet'], "text"),
    ...
    $insertSQL = sprintf("INSERT INTO ".$database_connect_prefix."activite_projet (intitule,niveau,code,parent,code_relai,structure,projet, id_personnel) VALUES (%s, %s, %s, %s, %s, %s, %s, '$personnel')",
                         ...
                         GetSQLValueString($_POST['code_relai'], "text"),
                         GetSQLValueString($_SESSION['clp_structure'], "text"),
                         GetSQLValueString($_SESSION['clp_projet'], "text"));
        try{
                $Result1 = $pdar_connexion->prepare($insertSQL);
  ...
  if ((isset($_POST["MM_update"]) && !empty($_POST["MM_update"]))) {
    ...
    $query_liste_actpa = sprintf("SELECT code FROM ".$database_connect_prefix."activite_projet WHERE structure=%s and projet=%s and code=%s",
    GetSQLValueString($_SESSION['clp_structure'], "text"),
    GetSQLValueString($_SESSION['clp_projet'], "text"),
    GetSQLValueString($_POST['code'], "text"));
    ...
    //structure=%s, GetSQLValueString($_SESSION['clp_structure'], "text"),
  	$insertSQL = sprintf("UPDATE ".$database_connect_prefix."activite_projet SET intitule=%s, niveau=%s, code=%s, parent=%s, code_relai=%s, projet=%s, modifier_par='$personnel', modifier_le='$date' WHERE structure=%s and projet=%s and id='$c'",
                         GetSQLValueString($_POST['intitule'], "text"),
                         GetSQLValueString($_POST['niveau'], "int"),
  					     GetSQLValueString($_POST['code'], "text"),
                         GetSQLValueString($_POST['parent'], "text"),
...
if ((isset($_POST["MM_form"])) && ($_POST["MM_form"] == "form2"))
{
  ...
  $form= $_POST['form']; $noms= $form['lib']; $codes= $form['code']; $nombre=count($noms);
    $insertSQL = sprintf("INSERT INTO ".$database_connect_prefix."niveau_config (nombre, libelle, code_number, structure, projet) VALUES (%s, %s, %s, %s, %s)",
  ...
  $date=date("Y-m-d"); $personnel=$_SESSION['clp_id'];
  $form= $_POST['form']; $noms= $form['lib']; $codes= $form['code']; $nombre=count($noms);
    $query_liste_actpa = sprintf("SELECT nombre FROM ".$database_connect_prefix."niveau_config WHERE structure=%s and projet=%s",
    GetSQLValueString($_SESSION['clp_structure'], "text"),
    GetSQLValueString($_SESSION['clp_projet'], "text"));
    try{
        $liste_actpa = $pdar_connexion->prepare($query_liste_actpa);
    ...
    if($totalRows_liste_actpa>0 && $row_liste_actpa["nombre"]==$nombre+1){
      $insertSQL = sprintf("DELETE from ".$database_connect_prefix."activite_projet WHERE niveau=%s and structure=%s and projet=%s",
  	...
  	$insertSQL = sprintf("UPDATE ".$database_connect_prefix."niveau_config SET nombre=%s, libelle=%s, code_number=%s WHERE projet=%s",
  					   GetSQLValueString($nombre, "int"),
  					   GetSQLValueString(implode(',',$noms), "text"),
                       GetSQLValueString(implode(',',$codes), "text"),
                       GetSQLValueString($_SESSION['clp_projet'], "text"));
...
if(isset($niveau)){ $lib = (isset($libelle[$niveau]))?$libelle[$niveau]:0; if($niveau<count($libelle)) {
  echo do_link("","","Importation d&eacute;puis un format excel $lib"," Importer $lib ","add","./","pull-right p11","get_content('import.php','id=plan_analytique&niveau=".($niveau+1)."','modal-body_add',this.title);",1,"",$nfile);
  echo do_link("new_plan","","Ajout $lib"," Ajouter $lib ","add","./","pull-right p11","get_content('new_activite_projet.php','niveau=".($niveau+1)."','modal-body_add',this.title);",1,"",$nfile);
  //echo do_link("","plan_analytique_projet.php","Retour","Retour","","./","pull-right p11","",0,"",$nfile);
  }
}
  else
  echo do_link("","","Importation d&eacute;puis un format excel"," Importer ","add","./","pull-right p11","get_content('import.php','id=plan_analytique&niveau=-1','modal-body_add',this.title);",1,"",$nfile);
if(isset($_SESSION["clp_niveau"]) && $_SESSION["clp_niveau"]==0)
  echo do_link("","","G&eacute;rer les niveaux", "&nbsp;&nbsp;Gestion des niveaux&nbsp;&nbsp;","add","./","pull-right p11","get_content('new_niveau.php','','modal-body_add',this.title);",1,"",$nfile);
    } }
?>
</div>
<div class="widget-content" style="display: block;">
<?php if(!isset($niveau)){
if(isset($libelle[0]) && !empty($libelle[0]) && !isset($_GET["down"])){  ?>
...
<!--                  <td align="center" colspan="<?php echo count($libelle)+1; ?>"><div class="Style8"><b>Code</b></div></td>      -->
                 ...
                 <?php if(isset($_SESSION['clp_niveau']) && $_SESSION['clp_niveau']<2 && !isset($_GET["down"])) {?>
...
function trace_tr($niveau,$j,$n,$libelle,$libelle1,$session,$nfile="")
{
  ...
  $data .= "<td colspan='".($n-$j+1)."'><b><!--".$libelle." -->".$libelle1["code"]." :</b> ".$libelle1["intitule"].":<b style=\"color:#CC0000\"> ".$libelle1["code_relai"]."</b></td>";
if($session<2) {
$data .= "<td align='center' width='80'>";
$data .= do_link("","","Modifier $libelle","","edit","./","","get_content('new_activite_projet.php','id=$id&niveau=".$libelle1['niveau']."','modal-body_add',this.title);",1,"margin:0px 5px;",$nfile);
...
//Localite
/*mysql_select_db($database_pdar_connexion, $pdar_connexion);
$query_projet = "SELECT * FROM ".$database_connect_prefix."activite_projet WHERE niveau<=$niveau and projet='".$_SESSION["clp_projet"]."' ";
...
if($totalRows_projet>0){  do{
  if(!isset($liste_loc_array[$row_projet["niveau"]])) $liste_loc_array[$row_projet["niveau"]] = array();
  $liste_loc_array[$row_projet["niveau"]][$row_projet["code"]] = array($row_projet["parent"],$row_projet["intitule"]);
}while($row_projet = mysql_fetch_assoc($projet));  }   */

$query_liste_activite_1 = "SELECT * FROM ".$database_connect_prefix."activite_projet WHERE $where and projet='".$_SESSION["clp_projet"]."' ORDER BY niveau,code ASC";
try{
    $liste_activite_1 = $pdar_connexion->prepare($query_liste_activite_1);
    $liste_activite_1->execute();
    $row_liste_activite_1 = $liste_activite_1 ->fetchAll();
...
<table id="example" border="0" align="center" cellspacing="0" class="table table-striped table-bordered table-hover table-responsive table-checkable table-colvis datatable dataTable" >
<?php if(count($libelle)>0 && $niveau<count($libelle)){ ?>
                <thead>
                  <tr>
                    ...
                    <?php if(isset($_SESSION['clp_niveau']) && $_SESSION['clp_niveau']<2 && !isset($_GET["down"])) {?>
					 <?php if(isset($row_entete["nombre"]) && $row_entete["nombre"]==$niveau+1 ) { ?>
				    <td width="120"><div align="center"><strong>Indicateurs</strong></div></td>
					  <?php } ?>
...
<?php if($totalRows_liste_activite_1>0){ foreach($row_liste_activite_1 as $row_liste_activite_1){ $id = $row_liste_activite_1["id"]; $code = $row_liste_activite_1["code"]; $parent = $row_liste_activite_1["parent"]; ?>
                <tr>
                    ...
                    <?php if(isset($_SESSION['clp_niveau']) && $_SESSION['clp_niveau']==0 && !isset($_GET["down"])) {?>
					 <?php if(isset($row_entete["nombre"]) && $row_entete["nombre"]==$niveau+1 ) { ?>
                    <td class=" " align="center"><strong>
					<a onclick="get_content('plan_indicateur_ppa.php','<?php echo "id_act=".$row_liste_activite_1['code']; ?>','modal-body_add','<?php echo $row_liste_activite_1["code"].": ".$row_liste_activite_1['intitule']; ?>','iframe');" data-toggle="modal" href="#myModal_add" data-backdrop="static" data-keyboard="false"  title="" class="thickbox" dir="">Indicateurs</a>
                    </strong></td>
					 <?php } ?>
                    <td class=" " align="center">
                    <?php   if(isset($_SESSION['clp_niveau']) && $_SESSION['clp_niveau']==0 && !isset($_GET["down"])) {
                    echo do_link("","","Modifier ".$libelle[$niveau],"","edit","./","","get_content('new_activite_projet.php','id=$id&niveau=".($niveau+1)."','modal-body_add',this.title);",1,"margin:0px 5px;",$nfile);
