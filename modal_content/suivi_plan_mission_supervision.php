...
mysql_select_db($database_pdar_connexion, $pdar_connexion);
$query_edit_recm = "SELECT * FROM ".$database_connect_prefix."recommandation_mission, ".$database_connect_prefix."mission_supervision WHERE id_mission=mission and id_recommandation='$rec' and  mission='$idms'";
$edit_recm = mysql_query_ruche($query_edit_recm, $pdar_connexion) or die(mysql_error_show_message(mysql_error()));
$row_edit_recm = mysql_fetch_assoc($edit_recm);
$totalRows_edit_recm = mysql_num_rows($edit_recm);

mysql_select_db($database_pdar_connexion, $pdar_connexion);
$query_plan_dec = "SELECT * FROM ".$database_connect_prefix."mission_plan where code_rec='$rec' and code_ms='$idms' order by ordre";
$plan_dec  = mysql_query_ruche($query_plan_dec , $pdar_connexion) or die(mysql_error());
$row_plan_dec = mysql_fetch_assoc($plan_dec);
$totalRows_plan_dec  = mysql_num_rows($plan_dec);
...
<table style="border-collapse: collapse;" class="table table-striped table-bordered table-hover table-responsive datatable  hide_befor_load" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
              <thead>
                <tr role="row">
                  ...
                  <th class="" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" aria-label="Trier" width="40" >%</th>
                  ...
                  <th class="" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" aria-label="Trier" >&nbsp;</th>
              ...
              <?php $t = 0;if ($totalRows_plan_dec > 0) {$p1 = "j";$t = $tprop=0;$i = $j = 0;do {?>
                <tr class="<?php echo ($i%2==0)?"odd":"even"; ?>">
                  ...
                  <td><textarea class="form-control " cols="200" rows="2" type="text" name="phase_realiser" id="phase_realiser" <?php if($row_plan_dec['valider']==1){ ?> disabled="disabled" <?php } ?>><?php echo (!empty($row_plan_dec['phase_realiser'])?$row_plan_dec['phase_realiser']:$row_plan_dec['phase']); if($row_plan_dec['valider']==1) $tprop+=$row_plan_dec['proportion']; ?></textarea></td>
                  <td align="center"><?php echo "<b>".number_format($row_plan_dec['proportion'], 0, ',', ' ')."</b>";  ?></td>
                  <td><input class="form-control datepicker required" type="text" <?php if(($row_plan_dec['valider'])==1){ ?> disabled="disabled" <?php } ?> type="text" name="date_reelle" <?php if(($row_plan_dec['valider'])==0) {?>style="border-color:#FF0000"<?php }?> value="<?php if(isset($row_plan_dec['date_reelle'])) echo implode('/',array_reverse(explode('-',$row_plan_dec['date_reelle']))); else echo date("d/m/Y"); ?>" size="10" <?php if($row_plan_dec['valider']==1){ ?> disabled="disabled" <?php } ?> /></td>
...
<?php //} ?>
				 ...
				 <?php if($row_plan_dec['valider']==0) $i++; $j++; } while ($row_plan_dec = mysql_fetch_assoc($plan_dec)); ?>
