...
$date=date("Y-m-d"); $personnel=$_SESSION['clp_id'];
  ...
  $insertSQL = sprintf("INSERT INTO ".$database_connect_prefix."mission_plan (code_ms, code_rec, ordre, phase, proportion, date_prevue, responsable, observation, id_personnel, date_enregistrement) VALUES (%s, %s, %s, %s, %s, %s, %s, %s,'$personnel', '$date')",
  ...
  //$Result1 = mysql_query_ruche($insertSQL, $pdar_connexion) or die(header(sprintf("Location: %s", $_SERVER['PHP_SELF']."?id_ct=$id_ct&insert=no")));
...
$Result1 = mysql_query_ruche($query_sup_loc, $pdar_connexion) or die(mysql_error());
  $insertGoTo = (isset($_GET['page']))?$_GET['page']:$_SERVER['PHP_SELF'];
  if ($Result1) $insertGoTo .= "?rec=$rec&idms=$idms&annee=$annee&del=ok"; else $insertGoTo .= "?rec=$rec&idms=$idms&annee=$annee&del=no";
  header(sprintf("Location: %s", $insertGoTo)); exit();
}
...
$edit_plan = mysql_query_ruche($query_edit_plan, $pdar_connexion) or die(mysql_error());
$row_edit_plan = mysql_fetch_assoc($edit_plan);
$totalRows_edit_plan = mysql_num_rows($edit_plan);
}
 mysql_select_db($database_pdar_connexion, $pdar_connexion);
...
$tolp = 0;
if($totalRows_plan_dec>0){ do{
  $tolp+=$row_plan_dec['proportion'];;
}while($row_plan_dec = mysql_fetch_assoc($plan_dec));
  $rows = mysql_num_rows($plan_dec);
  if($rows > 0) {
      mysql_data_seek($plan_dec, 0);
   $row_plan_dec = mysql_fetch_assoc($plan_dec);
  }
}
?>
...
<?php } ?>
    ...
    <tbody role="alert" aria-live="polite" aria-relevant="all" class="hide_befor_load">
      <?php $t = 0;if ($totalRows_plan_dec > 0) {$p1 = "j";$t = 0;$i = 0;do {?>
...
<?php } while ($row_plan_dec = mysql_fetch_assoc($plan_dec)); ?>
  ...
  <textarea class="form-control required" id="responsable" name="responsable" cols="25" rows="1"><?php if(isset($_GET["id"]) && !empty($_GET["id"])) echo $row_edit_plan['responsable'];?></textarea>
    ...
    <tr valign="top">
      ...
      <td>
        <div class="form-group">
          <div class="col-md-12">
          <i style="color: red;">Reste: <?php echo 100-$tolp; ?>%</i>
          </div>
        </div>
      </td>
    </tr>
