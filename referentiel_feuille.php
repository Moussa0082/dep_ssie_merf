...
if(isset($_GET["idref"]) && intval($_GET["idref"])>0)
{
  $idref=intval($_GET["idref"]);
  mysql_select_db($database_pdar_connexion, $pdar_connexion);
  $query_edit_indicateur = "SELECT * FROM referentiel_indicateur WHERE id_ref_ind='$idref'";
  $edit_indicateur  = mysql_query($query_edit_indicateur , $pdar_connexion) or die(mysql_error());
...
if ((isset($_GET["id_sup"]) && !empty($_GET["id_sup"]))) {
  $id = ($_GET["id_sup"]);
  $insertSQL = sprintf("DELETE from ".$database_connect_prefix."referentiel_fiche_config WHERE id=%s",
                       GetSQLValueString($id, "int"));

  mysql_select_db($database_pdar_connexion, $pdar_connexion);
  $Result1 = mysql_query($insertSQL, $pdar_connexion) or die(mysql_error_show_message(mysql_error()));
  $insertGoTo = $_SERVER['PHP_SELF'];
  if ($Result1) $insertGoTo .= "?del=ok&idref=$idref"; else $insertGoTo .= "?del=no&idref=$idref";
...
if ((isset($_POST["MM_form"])) && ($_POST["MM_form"] == "form1"))
{
  ...
  $personnel=$_SESSION['clp_id']; $date=date("Y-m-d");
    $insertSQL = sprintf("INSERT INTO ".$database_connect_prefix."referentiel_fiche_config (referentiel, classeur, feuille, colonne, colonneC, mode_calcul, critere, etat, projet, id_personnel, date_enregistrement) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, '$personnel', '$date')",
                       ...
                       GetSQLValueString($_POST['colonneC'], "text"),
    ...
    $insertGoTo = $_SERVER['PHP_SELF'];
    if($Result1) $insertGoTo .= "?insert=ok&idref=$idref"; else $insertGoTo .= "?insert=no&idref=$idref";
    header(sprintf("Location: %s", $insertGoTo)); exit();
  }
  ...
  $date=date("Y-m-d"); $personnel=$_SESSION['clp_id'];  $id = ($_POST["MM_update"]);
    $insertSQL = sprintf("UPDATE ".$database_connect_prefix."referentiel_fiche_config SET classeur=%s, feuille=%s, colonne=%s, colonneC=%s, mode_calcul=%s, critere=%s, etat=%s, modifier_par='$personnel', modifier_le='$date' WHERE id=%s",
	                   GetSQLValueString($_POST['classeur'], "int"),
                       GetSQLValueString($_POST['feuille'], "text"),
					   GetSQLValueString($_POST['colonne'], "text"),
                       GetSQLValueString($_POST['colonneC'], "text"),
    ...
    $insertGoTo = $_SERVER['PHP_SELF'];
    if($Result1) $insertGoTo .= $page."?update=ok&idref=$idref"; else $insertGoTo .= $page."?update=no&idref=$idref";
    header(sprintf("Location: %s", $insertGoTo));  exit();
  }
}
...
mysql_select_db($database_pdar_connexion, $pdar_connexion);
$query_liste_conf = "SELECT h.*, c.couleur as couleur_c FROM ".$database_connect_prefix."referentiel_fiche_config h, ".$database_connect_prefix."classeur c WHERE c.id_classeur=h.classeur and referentiel=$idref";
...
<div class="widget-header"> <center><h4><?php if(isset($_GET["idref"])){ ?><b><u>Indicateur</u> : <?php echo $row_edit_indicateur['code_ref_ind'].": ".$row_edit_indicateur['intitule_ref_ind']; ?></b><?php } else { ?>Veuillez s&eacute;lectionner un indicateur<?php } ?> </h4></center></div>
<div class="widget-header"> <h4><i class="icon-reorder"></i> Configuration des fiches li&eacute;es</h4>
  <?php if(isset($_SESSION['clp_niveau']) && $_SESSION['clp_niveau']==1){ ?>
<?php
echo do_link("","","Ajout d'&eacute;l&eacute;ment","<i class=\"icon-plus\"> Nouveau lien </i>","","./","pull-right p11","get_content('new_fiche_referentiel.php?idref=$idref','','modal-body_add',this.title);",1,"",$nfile);
?>
<?php } ?>
<a href="referentiel.php" data-backdrop="static" class="pull-right p11" title="Retour" ><i class="icon-close"> Retour </i></a>
</div>
...
<th width="100" class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" aria-label="Trier">Mode de calcul </th>
<th width="100" class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" aria-label="Trier">Crit&egrave;res</th>
<?php if(isset($_SESSION['clp_niveau']) && ($_SESSION['clp_niveau']==1)) {?>
...
<td align="center"><span class=" "><?php echo $row_liste_conf['mode_calcul']; ?></span></td>
<td align="center"><span class=" "><?php echo $row_liste_conf['critere']; ?></span></td>
<?php if(isset($_SESSION['clp_niveau']) && ($_SESSION['clp_niveau']==1)) {?>
<td class=" " align="center">
<?php
echo do_link("","","Modifier d'&eacute;l&eacute;ment ","","edit","./","","get_content('new_fiche_referentiel.php?idref=$idref','id=$id','modal-body_add',this.title);",1,"margin:0px 5px;",$nfile);

echo do_link("",$_SERVER['PHP_SELF']."?id_sup=$id&idref=$idref","Supprimer","","del","./","","return confirm('Voulez-vous vraiment supprimer cet &eacute;l&eacute;ment ?');",0,"margin:0px 5px;",$nfile);
?></td>
<?php }?>
</tr>
