  ...
  if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "MM_insert"))
  {
    ...
    if(in_array($ext,$extensions_autorisees))
    {
      ...
      elseif(isset($_FILES['fichier']) && $_FILES['fichier']['error'] == 0)
      {
        ...
        $query_sup_import_annee = sprintf("DELETE from ".$database_connect_prefix."plan_budgetaire WHERE ".(($niveau1==-1)?"":" niveau=".GetSQLValueString(intval($_GET["niveau"])+1, "int")." and ")."structure=%s and projet=%s",
        ...
        for ($row = 5; $row <= $highestRow; $row++)
        {
            ...
            if(!empty($code) && $code!='Code')
            {
              ...
              if($niveau1==-1){
                ...
                $query_liste_activite_1 = "SELECT code FROM ".$database_connect_prefix."plan_budgetaire WHERE $where and ".$_SESSION["clp_where"]." ";
                try{
              ...
              $insertSQL = sprintf("INSERT INTO ".$database_connect_prefix."plan_budgetaire (intitule,niveau,code,parent,code_relai,structure,projet, id_personnel) VALUES (%s, %s, %s, %s, %s, %s, %s, '$personnel')",
                                  // GetSQLValueString(trim(utf8_encode($rowData[0][5])),"text"),
								   GetSQLValueString(trim(addslashes($rowData[0][5])), "text"),
                                   GetSQLValueString($n, "int"),
            					   GetSQLValueString($code, "text"),
                                   GetSQLValueString($p, "text"),
                                   GetSQLValueString(trim($rowData[0][6]), "text"),
                                   GetSQLValueString($_SESSION['clp_structure'], "text"),
                                   GetSQLValueString($_SESSION['clp_projet'], "text"));
              try{
              ...
              //echo $n." - ".$code." - ".trim($rowData[0][5])." - <font size='1'>".$insertSQL."</font><br />";
            }
          }
...
if ((isset($_POST["id_val"]) && !empty($_POST["id_val"]))) {
      ...
      $insertSQL = sprintf("DELETE from ".$database_connect_prefix."plan_budgetaire WHERE id in ($id)");
      try{
            $Result1 = $pdar_connexion->prepare($insertSQL);
            $Result1->execute();
        }catch(Exception $e){ die(mysql_error_show_message($e)); }
...
if ((isset($_GET["id_sup"]) && !empty($_GET["id_sup"]))) {
      $id = ($_GET["id_sup"]);
      $insertSQL = sprintf("DELETE from ".$database_connect_prefix."plan_budgetaire WHERE id=%s",
                           GetSQLValueString($id, "in"));
      try{
            $Result1 = $pdar_connexion->prepare($insertSQL);
            $Result1->execute();
        }catch(Exception $e){ die(mysql_error_show_message($e)); }
...
if ((isset($_POST["MM_form"])) && ($_POST["MM_form"] == "form1"))
{
  if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "MM_insert")) {
   $personnel=$_SESSION['clp_id'];
    $query_liste_actpa = sprintf("SELECT code FROM ".$database_connect_prefix."plan_budgetaire WHERE structure=%s and projet=%s and code=%s",
            GetSQLValueString($_SESSION['clp_structure'], "text"),
            GetSQLValueString($_SESSION['clp_projet'], "text"),
  ...
  if ((isset($_POST["MM_update"]) && !empty($_POST["MM_update"]))) {
    ...
    //structure=%s, GetSQLValueString($_SESSION['clp_structure'], "text"),
  	$insertSQL = sprintf("UPDATE ".$database_connect_prefix."plan_budgetaire SET intitule=%s, niveau=%s, code=%s, parent=%s, code_relai=%s, projet=%s, modifier_par='$personnel', modifier_le='$date' WHERE structure=%s and projet=%s and id='$c'",
                         GetSQLValueString($_POST['intitule'], "text"),
                         GetSQLValueString($_POST['niveau'], "int"),
  					     GetSQLValueString($_POST['code'], "text"),
                         GetSQLValueString($_POST['parent'], "text"),
...
if ((isset($_POST["MM_form"])) && ($_POST["MM_form"] == "form2"))
{
  ...
  $form= $_POST['form']; $noms= $form['lib']; $codes= $form['code']; $nombre=count($noms);
    $insertSQL = sprintf("INSERT INTO ".$database_connect_prefix."niveau_budget_config (nombre, libelle, code_number, structure, projet) VALUES (%s, %s, %s, %s, %s)",
  ...
  $date=date("Y-m-d"); $personnel=$_SESSION['clp_id'];
  $form= $_POST['form']; $noms= $form['lib']; $codes= $form['code']; $nombre=count($noms);
    $query_liste_actpa = sprintf("SELECT nombre FROM ".$database_connect_prefix."niveau_budget_config WHERE structure=%s and projet=%s",
            GetSQLValueString($_SESSION['clp_structure'], "text"),
            GetSQLValueString($_SESSION['clp_projet'], "text"));
    try{
        $liste_actpa = $pdar_connexion->prepare($query_liste_actpa);
    ...
    if($totalRows_liste_actpa>0 && $row_liste_actpa["nombre"]==$nombre+1){
      $insertSQL = sprintf("DELETE from ".$database_connect_prefix."plan_budgetaire WHERE niveau=%s and structure=%s and projet=%s",
  	...
  	$insertSQL = sprintf("UPDATE ".$database_connect_prefix."niveau_budget_config SET nombre=%s, libelle=%s, code_number=%s WHERE structure=%s and projet=%s",
  					   GetSQLValueString($nombre, "int"),
  					   GetSQLValueString(implode(',',$noms), "text"),
                       GetSQLValueString(implode(',',$codes), "text"),
                       GetSQLValueString($_SESSION['clp_structure'], "text"),
  ...
  $query_liste_activite = "SELECT * FROM ".$database_connect_prefix."plan_budgetaire WHERE niveau =1 and projet='".$_SESSION["clp_projet"]."' ORDER BY niveau,code ASC";
try{
...
<div class="widget-header"> <h4><i class="icon-reorder"></i> Plan Budgetaire </h4>
<?php //if(!isset($niveau)){
...
if(isset($niveau)){ $lib = (isset($libelle[$niveau]))?$libelle[$niveau]:0; if($niveau<count($libelle)) {
  ...
  echo do_link("","","Ajout $lib","<i class=\"icon-plus\"> Ajouter $lib </i>","","./","pull-right p11","get_content('new_plan_budgetaire.php','niveau=".($niveau+1)."','modal-body_add',this.title);",1,"",$nfile);
...
if(isset($_SESSION["clp_niveau"]) && $_SESSION["clp_niveau"]==0)
  echo do_link("","","G&eacute;rer les niveaux","<i class=\"icon-plus\"> Gestion des niveaux </i>","","./","pull-right p11","get_content('new_budget_niveau.php','','modal-body_add',this.title);",1,"",$nfile);
    }
?>
</div>
...
<!--                  <td align="center" colspan="<?php echo count($libelle)+1; ?>"><div class="Style8"><b>Code</b></div></td>      -->
                  <td colspan="<?php echo $n+1; ?>"><div align="left"><strong><span class="Style8">Libell&eacute;s </span></strong></div></td>
...
$data .= do_link("","","Modifier $libelle","","edit","./","","get_content('new_plan_budgetaire.php','id=$id&niveau=".$libelle1['niveau']."','modal-body_add',this.title);",1,"margin:0px 5px;",$nfile);
...
$niveau_indent = $n;   $k = 0;
$query_liste_activite_1 = "SELECT * FROM ".$database_connect_prefix."plan_budgetaire WHERE $where $wh and projet='".$_SESSION["clp_projet"]."' ORDER BY niveau,code ASC";
try{
    $liste_activite_1 = $pdar_connexion->prepare($query_liste_activite_1);
    $liste_activite_1->execute();
...
$query_liste_activite_2 = "SELECT * FROM ".$database_connect_prefix."plan_budgetaire WHERE niveau=".($j+2)." and parent='$code_1' and projet='".$_SESSION["clp_projet"]."' ORDER BY niveau,code ASC";
try{
...
$query_liste_activite_3 = "SELECT * FROM ".$database_connect_prefix."plan_budgetaire WHERE niveau=".($j+2)." and parent='$code_2' and projet='".$_SESSION["clp_projet"]."' ORDER BY niveau,code ASC";
try{
...
$query_liste_activite_4 = "SELECT * FROM ".$database_connect_prefix."plan_budgetaire WHERE niveau=".($j+2)." and parent='$code_3' and projet='".$_SESSION["clp_projet"]."' ORDER BY niveau,code ASC";
try{
...
$query_liste_activite_5 = "SELECT * FROM ".$database_connect_prefix."plan_budgetaire WHERE niveau=".($j+2)." and parent='$code_4' and projet='".$_SESSION["clp_projet"]."' ORDER BY niveau,code ASC";
try{
...
$query_liste_activite_6 = "SELECT * FROM ".$database_connect_prefix."plan_budgetaire WHERE niveau=".($j+2)." and parent='$code_5' and projet='".$_SESSION["clp_projet"]."' ORDER BY niveau,code ASC";
try{
...
<?php if($totalRows_liste_activite_1>0){ foreach($row_liste_activite_1 as $row_liste_activite_1){ $id = $row_liste_activite_1["id"]; $code = $row_liste_activite_1["code"]; $parent = $row_liste_activite_1["parent"]; ?>
                <tr>
                    ...
                    <?php
if(isset($_SESSION["clp_niveau"]) && $_SESSION["clp_niveau"]==0){
                    echo do_link("","","Modifier ".$libelle[$niveau],"","edit","./","","get_content('new_plan_budgetaire.php','id=$id&niveau=".($niveau+1)."','modal-body_add',this.title);",1,"margin:0px 5px;",$nfile);
