...
//Structure
$query_structure = "SELECT * FROM ".$database_connect_prefix."structure ";
try{
...
foreach($ligne as $ligne){$tableauStructure[]=$ligne['code_structure']."<>".$ligne['sigle']; $tableauStructureV[$ligne['code_structure']]=$ligne['nom_structure'];}
...
<table border="0" id="mtable" align="center" cellspacing="1" cellpadding="0" width="100%" style="font-size:14px;">
    ...
    <tr valign="top" bgcolor="#FFFFCC">
      <td colspan="2">
        <div class="form-group">
          <label for="ugl" class="col-md-3 control-label">Entit&eacute; de gestion <span class="required">*</span></label>
          <div class="col-md-9">
          <?php
          if(isset($_GET["id"]) && intval($_GET["id"])>0) $a = explode("|", $row_liste_projet['ugl']); ?>
          <table width="100%">
          <tr>
          <?php $i=1; foreach($tableauUgl as $vugl){?>
          <?php
          $augl = explode('<>',$vugl);
          $iugl = $augl[0];
          ?>
          <td><label title="<?php echo $tableauUglV[$iugl]; ?>" for="ugl_<?php echo $i; ?>" class="control-label"><?php if(isset($augl[1])) echo $augl[1]; ?></label>
          <input title="<?php echo $tableauUglV[$iugl]; ?>" name='ugl[]' id='ugl_<?php echo $i; ?>' type="checkbox"   <?php if(isset($_GET['id'])) { if(in_array($iugl, $a, TRUE)) echo "checked"; }?> size="5" value="<?php if(isset($iugl)) echo $iugl; ?>"/></td>
          <?php $i++; } ?>
          </tr>
          </table>
          </div>
