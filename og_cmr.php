...
if ((isset($_POST["MM_form"])) && ($_POST["MM_form"] == "form1"))
{
  ...
  $date=date("Y-m-d"); $personnel=$_SESSION['clp_id']; $acteur="";
    ...
    $insertSQL = sprintf("INSERT INTO indicateur_objectif_global_cmr (indicateur_og, referentiel, intitule_indicateur_cmr_og, cible_cmr,annee_reference,reference_cmr, responsable_collecte, personnel, date_enregistrement) VALUES (%s, %s, %s, %s, %s, %s, %s, '$personnel', '$date')",
 ...
 try{
  ...
  if ((isset($_POST["MM_delete"]) && intval($_POST["MM_delete"])>0)) {
      ...
      $insertSQL = sprintf("DELETE from indicateur_objectif_global_cmr WHERE id_indicateur=%s",
 ...
 try{
  ...
  $date=date("Y-m-d"); $personnel=$_SESSION['clp_id']; $c=$_POST['id']; $acteur="";
  	if(!empty($_POST['acteur'])) { foreach($_POST['acteur'] as $vacteur) { $acteur=$acteur.",".$vacteur; } }
    $insertSQL = sprintf("UPDATE indicateur_objectif_global_cmr SET  indicateur_og=%s, referentiel=%s, intitule_indicateur_cmr_og=%s, cible_cmr=%s, annee_reference=%s, reference_cmr=%s, responsable_collecte=%s, etat='Modifié', modifier_par='$personnel', modifier_le='$date' WHERE id_indicateur='$c'",
  					   GetSQLValueString($_POST['indicateur_cl'], "int"),
                         GetSQLValueString((isset($_POST['referentiel'])?$_POST['referentiel']:0), "int"),
  	   				   GetSQLValueString($_POST['indicateur'], "text"),
  					   GetSQLValueString($_POST['cible_dp'], "text"),
...
$query_liste_composante = "SELECT indicateur_objectif_global_cmr.*, indicateur_objectif_global.* FROM indicateur_objectif_global_cmr, indicateur_objectif_global WHERE indicateur_objectif_global.projet='".$_SESSION["clp_projet"]."' and id_indicateur_objectif_global=indicateur_og  order by id_indicateur_objectif_global, code_cmr";
try{
...
$query_edit_nom = "SELECT id_indicateur_objectif_global, code_iog, intitule_indicateur_objectif_global  FROM indicateur_objectif_global WHERE projet='".$_SESSION["clp_projet"]."' order by code_iog";
try{
...
$code_resultat_array[$row_edit_nom["id_indicateur_objectif_global"]]=$row_edit_nom["code_iog"];
} }

if(isset($_GET["id_sup"])) { $id=$_GET["id_sup"];
$query_sup_activite = "DELETE FROM indicateur_objectif_global_cmr WHERE id_indicateur='$id'";
	   try{
...
$cible_array = $ciblem_array = array();
 if($totalRows_cible_indicateur>0) { foreach($row_cible_indicateur as $row_cible_indicateur){  
  $cible_array[$row_cible_indicateur["indicateur_og"]][$row_cible_indicateur["annee"]]=$row_cible_indicateur["valeur_cible"];
  $ciblem_array[$row_cible_indicateur["indicateur_og"]][$row_cible_indicateur["annee"]]=$row_cible_indicateur["valeur_ciblem"];
   }}
...
<div class="widget-header"> <h4><i class="icon-reorder"></i> Cadre de Mesure de R&eacute;sultats (<span style="color:#FFFF00">Impacts</span>) </h4>
    <?php if(isset($_SESSION['clp_niveau']) && $_SESSION['clp_niveau']<2){?>
<a onclick="get_content('edit_indicateur_ogcmr.php','','modal-body_add',this.title);" data-toggle="modal" href="#myModal_add" data-backdrop="static" data-keyboard="false" title="Ajout d'indicateur d'Impact" class="btn btn-sm btn-warning pull-right p11" dir=""><i class="icon-plus"> Nouvel indicateur </i></a><a href="odp_cmr.php" title="Editer les objectifs de d&eacute;veloppement" class="pull-right p11"> Objectif de D&eacute;veloppement </a>
...
<th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" aria-label="Trier">Objectif</th>
...
<th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" aria-label="Trier">R&eacute;f&eacute;rence</th>
...
<td class=" "><?php if(isset($code_resultat_array[$row_liste_composante['indicateur_og']])) echo $code_resultat_array[$row_liste_composante['indicateur_og']]; ?></td>
<td class=" "><?php echo (!empty($row_liste_composante['intitule_indicateur_cmr_og']))?$row_liste_composante['intitule_indicateur_cmr_og']:""; ?></td>
...
<td class=" "><?php echo (!empty($row_liste_composante['reference_cmr']))?$row_liste_composante['reference_cmr']:""; ?></td>
<td class=" " align="center"><span class="Style22">
 ...
 <?php for($i=$_SESSION["annee_debut_projet"]; $i<=$_SESSION["annee_fin_projet"]; $i++){ ?>
    ...
    <?php } ?>
                       <td class=" " align="center"> <a onclick="get_content('./edit_cible_indicateur_og.php','<?php echo "id_ind=".$row_liste_composante['id_indicateur']."&code_act=".$row_liste_composante['code_cmr']; ?>','modal-body_add',this.title,'iframe');" data-toggle="modal" href="#myModal_add" data-backdrop="static" data-keyboard="false" title="'<?php echo str_replace("'","\'",$row_liste_composante['intitule_indicateur_cmr_og']);?>'" class="thickbox" dir="">Cible</a></td>
                   
<?php if(isset($_SESSION['clp_niveau']) && $_SESSION['clp_niveau']==1) { ?>
<td class=" " align="center">
<?php
echo do_link("","","Modifier Indicateur ".$row_liste_composante['intitule_indicateur_cmr_og'],"","edit","./","","get_content('edit_indicateur_ogcmr.php','id=$id','modal-body_add',this.title);",1,"margin:0px 5px;",$nfile);
