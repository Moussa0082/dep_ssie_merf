...
$query_edit_ind = "SELECT * FROM indicateur_objectif_specifique_cmr WHERE id_indicateur='$id'";
try{
...
//indicateur resultat
$query_liste_ind_cl = "SELECT id_indicateur_objectif_specifique, intitule_indicateur_objectif_specifique FROM indicateur_objectif_specifique,objectif_specifique WHERE objectif_specifique.projet='".$_SESSION["clp_projet"]."' and objectif_specifique=id_objectif_specifique";
try{
...
$query_liste_referentiel = "SELECT * FROM referentiel_indicateur WHERE (type_ref_ind=2 OR type_ref_ind=3)";
try{
...
$query_liste_indicateur = "SELECT * FROM indicateur_objectif_specifique,objectif_specifique WHERE projet='".$_SESSION["clp_projet"]."' and objectif_specifique=id_objectif_specifique";
...
<div class="widget-header"> <h4><i class="icon-reorder"></i> <?php echo (isset($_GET["id"]) && intval($_GET["id"])>0)?"Modification indicateur ODP du CMR":"Nouveau indicateur ODP du CMR"?></h4> </div>
...
<table border="0" id="mtable" align="center" cellspacing="1" cellpadding="0" width="100%" style="font-size:14px;">
    ...
    <tr valign="top">
      <td colspan="2">
        <div class="form-group">
          <label for="indicateur" class="col-md-3 control-label">Indicateur ODP <span class="required">*</span></label>
          <div class="col-md-9">
            <textarea name="indicateur" cols="32" rows="2" class="form-control required" id="indicateur"><?php echo (isset($row_edit_ind['intitule_indicateur_cmr_os']))?$row_edit_ind['intitule_indicateur_cmr_os']:""; ?></textarea>
          </div>
        </div>
      </td>
    </tr>
    ...
    <tr valign="top">
      <td colspan="2">
        <div class="form-group">
          <label for="indicateur_cl" class="col-md-3 control-label">Indicateur os <span class="required">*</span></label>
          <div class="col-md-9">
            <select name="indicateur_cl" id="indicateur_cl" class="form-control required" >
              ...
              <option value="<?php echo $row_liste_indicateur['id_indicateur_objectif_specifique']; ?>" <?php if (isset($row_edit_ind['indicateur_os']) && $row_liste_indicateur['id_indicateur_objectif_specifique']==$row_edit_ind['indicateur_os']) {echo "SELECTED";} ?>><?php echo (strlen($row_liste_indicateur['intitule_indicateur_objectif_specifique'])>55)?substr($row_liste_indicateur['intitule_indicateur_objectif_specifique'],0,55)." ...".substr($row_liste_indicateur['intitule_indicateur_objectif_specifique'],-10,10):$row_liste_indicateur['intitule_indicateur_objectif_specifique']; ?></option>
              <?php } } ?>
            </select>
          </div>
        </div>
    ...
    <tr valign="top">
      <td colspan="2">
        <div class="form-group">
          ...
          <label for="reference_cmr" class="col-md-3 control-label">Situation de r&eacute;f&eacute;rence <span class="required">*</span></label>
          <div class="col-md-3">
