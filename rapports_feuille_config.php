...
if ((isset($_GET["id_sup"]) && !empty($_GET["id_sup"]))) {
  $id = ($_GET["id_sup"]);
  $insertSQL = sprintf("DELETE from ".$database_connect_prefix."rapport_fiche_config WHERE id=%s",
...
if ((isset($_POST["MM_form"])) && ($_POST["MM_form"] == "form1") && isset($_POST['colonne']) && isset($_POST['colonneV']) && !empty($_POST['colonne']) && !empty($_POST['colonneV']))
{ $_POST['feuille']="";
  if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "MM_insert")) {
  $personnel=$_SESSION['clp_id']; $date=date("Y-m-d"); $show_colonne=implode(";",$_POST['show_colonne']);
    $insertSQL = sprintf("INSERT INTO ".$database_connect_prefix."rapport_fiche_config (intitule, classeur, feuille, colonne, colonneV, mode, show_colonne, critere_in, critere_not_in, projet, id_personnel, date_enregistrement) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, '$personnel', '$date')",
					   ...
					   GetSQLValueString($_POST['critere_in'], "text"),
                       GetSQLValueString($_POST['critere_not_in'], "text"),
  ...
  $date=date("Y-m-d"); $personnel=$_SESSION['clp_id'];  $id = ($_POST["MM_update"]);  $show_colonne=implode(";",$_POST['show_colonne']);
    $insertSQL = sprintf("UPDATE ".$database_connect_prefix."rapport_fiche_config SET intitule=%s, classeur=%s, feuille=%s, colonne=%s, colonneV=%s, mode=%s, show_colonne=%s, critere_in=%s, critere_not_in=%s, etat='Modifi̩', modifier_par='$personnel', modifier_le='$date' WHERE id=%s",
                       GetSQLValueString($_POST['intitule'], "text"),
	                   GetSQLValueString($_POST['classeur'], "int"),
                       GetSQLValueString($_POST['feuille'], "text"),
					   GetSQLValueString($_POST['colonne'], "text"),
...
mysql_select_db($database_pdar_connexion, $pdar_connexion);
$query_liste_conf = "SELECT h.*, c.couleur as couleur_c FROM ".$database_connect_prefix."rapport_fiche_config h, ".$database_connect_prefix."classeur c WHERE c.id_classeur=h.classeur ";
...
<div class="widget-header"> <h4><i class="icon-reorder"></i> Configuration des rapports dynamiques &agrave; afficher sur Rapports Dynamiques</h4>

<?php if(isset($_SESSION['clp_niveau']) && $_SESSION['clp_niveau']==1){ ?>
<?php
echo do_link("","","Ajout d'&eacute;l&eacute;ment","<i class=\"icon-plus\"> Nouveau &eacute;l&eacute;ment </i>","","./","pull-right p11","get_content('new_rapport_feuille_simple.php','','modal-body_add',this.title);",1,"",$nfile);
?>
<?php } ?>
<a href="rapports_dynamiques.php" title="Retour" class="pull-right p11"><i class="icon-close">&nbsp;Retour </i></a>
</div>
...
<thead>
<tr role="row">
  <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" aria-label="Trier">Intitul&eacute;</th>
...
<th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" aria-label="Trier">R&eacute;grouper par</th>
<th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" aria-label="Trier">Colonne valeur</th>
...
<th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" aria-label="Trier">Crit&egrave;re &agrave; inclure</th>
<th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" aria-label="Trier">Crit&egrave;re &agrave; exclure</th>
<?php if(isset($_SESSION['clp_niveau']) && ($_SESSION['clp_niveau']==1)) {?>
...
foreach($ff as $f){ $f=explode("/",$f);  if(isset($f[1])) $fv[$f[1]]=$f[1]; }
foreach($fv as $nomtab){ echo (isset($entete_array[$nomtab]))?$entete_array[$nomtab]:'NaN'; echo "<br />"; }

 ?></td>
<td align="left" nowrap="nowrap"><?php $f=explode("/",$row_liste_conf['colonne']); echo (isset($libelle_array[$f[1]][$f[0]]))?$libelle_array[$f[1]][$f[0]]:$f[0]; ?></td>
...
foreach($ff as $f){ $f=explode("/",$f);  if(isset($f[0])) $fv[$f[0]]=$f[0]; }
foreach($fv as $nomtab){ echo (isset($libelle_array[$nomtab]))?$libelle_array[$nomtab]:$nomtab; echo "<br />"; }

 ?></td>
<td class=" "><?php echo $row_liste_conf['critere_in']; ?></td>
<td class=" "><?php echo $row_liste_conf['critere_not_in']; ?></td>
<?php if(isset($_SESSION['clp_niveau']) && ($_SESSION['clp_niveau']==1)) {?>
<td class=" " align="center">
<?php
echo do_link("","","Modifier d'&eacute;l&eacute;ment ","","edit","./","","get_content('new_rapport_feuille_simple.php','id=$id','modal-body_add',this.title);",1,"margin:0px 5px;",$nfile);
