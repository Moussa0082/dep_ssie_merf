...
{ //Objectif Global
 /* if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "MM_insert")) {
    $insertSQL = sprintf("INSERT INTO objectif_global (intitule_objectif_global, code_og, structure, projet, id_personnel, date_enregistrement) VALUES (%s, %s, %s, %s, '$personnel', '$date')",
  ...
  */
  ...
  $date=date("Y-m-d"); $personnel=$_SESSION['clp_id'];  $id = intval($_POST["MM_update"]);
...
{ //Indicateur Objectif Global
  if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "MM_insert")) {
    $insertSQL = sprintf("INSERT INTO indicateur_objectif_global (intitule_indicateur_objectif_global, code_iog, unite, reference, mi_parcours, cible_dp, source, periodicite, responsable,projet, id_personnel, date_enregistrement) VALUES (%s,%s, %s, %s,%s, %s, %s,%s, %s, %s, '$personnel', '$date')",
                         GetSQLValueString($_POST['nom'], "text"),
                         GetSQLValueString($_POST['code'], "text"),
 ...
 // }
  ...
  header(sprintf("Location: %s", $insertGoTo)); exit(0);
      ...
      if ((isset($_POST["MM_update"]) && !empty($_POST["MM_update"]))) {
  $date=date("Y-m-d"); $personnel=$_SESSION['clp_id'];  $id = intval($_POST["MM_update"]);
    $insertSQL = sprintf("UPDATE indicateur_objectif_global SET intitule_indicateur_objectif_global=%s, code_iog=%s, unite=%s, reference=%s, mi_parcours=%s, cible_dp=%s, source=%s, periodicite=%s, responsable=%s, etat='Modifié', modifier_par='$personnel', modifier_le='$date' WHERE id_indicateur_objectif_global=%s",
                         GetSQLValueString($_POST['nom'], "text"),
                         GetSQLValueString($_POST['code'], "text"),
						 GetSQLValueString($_POST['unite'], "text"),
						 GetSQLValueString($_POST['reference'], "double"),
...
{ //Source Objectif Global
  if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "MM_insert")) {
    $insertSQL = sprintf("INSERT INTO source_og (intitule_source, projet, id_personnel, date_enregistrement) VALUES (%s, %s, '$personnel', '$date')",
                         GetSQLValueString($_POST['nom'], "text"),
                        // GetSQLValueString($_SESSION['clp_structure'], "text"),
                         GetSQLValueString($_SESSION['clp_projet'], "text"));
...
{ //Hypothèse Objectif Global
  if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "MM_insert")) {
    $insertSQL = sprintf("INSERT INTO hypothese_og (intitule_hypothese, projet, id_personnel, date_enregistrement) VALUES (%s, %s, '$personnel', '$date')",
                         GetSQLValueString($_POST['nom'], "text"),
                        // GetSQLValueString($_SESSION['clp_structure'], "text"),
                         GetSQLValueString($_SESSION['clp_projet'], "text"));
...
<div class="widget-header"> <h4><i class="icon-reorder"></i> Cadre Logique : <span style="color:#FFFF00">Edition de l'objectif global</span></h4>
...
<div class="well well-sm"><strong> 1. <span class="Style22">OBJECTIF GLOBAL</span> (But) </strong></div>
...
<table width="100%" border="0" align="center" cellspacing="1" class="table table-striped table-bordered table-responsive">
              ...
              <tr>
				...
				<?php if($totalRows_og>0) { $i=0; foreach($row_og as $row_og){ $id = $row_og['id_objectif_global'];  ?>
...
<a onclick="get_content('new_objectif_global.php','id=<?php echo $id;?>','modal-body_add',this.title);" data-toggle="modal" href="#myModal_add" title="Modifier" class="thickbox Add"  dir=""><span style=""><span class="Style10">
                ...
                <td valign="top"><table border="0" cellspacing="0">
					<?php if($totalRows_ind>0) { $i=0; foreach($row_ind as $row_ind){ $id = $row_ind["id_indicateur_objectif_global"]; ?>
                    <tr <?php if ($i % 2 == 0) echo 'bgcolor="#ECF0DF"';$i = $i + 1;?>>
                      <td ><div align="left"><?php echo $i;?>
<a onclick="get_content('new_indicateur_objectif_global.php','id=<?php echo $id;?>','modal-body_add',this.title);" data-toggle="modal" href="#myModal_add" title="Modifier" class="thickbox Add"  dir=""><span style=""><span class="Style10">
                <?php echo "- " . $row_ind['intitule_indicateur_objectif_global'];?>
