...
{
  $id=($_GET["id"]);
  $query_liste_activite = "SELECT * FROM ".$database_connect_prefix."plan_budgetaire WHERE id='$id'";
  try{
...
$query_liste_volet = "SELECT * FROM ".$database_connect_prefix."plan_budgetaire WHERE projet='".$_SESSION["clp_projet"]."' and $where ORDER BY niveau,code ASC";
try{
    ...
    <tr valign="top">
      <td>
        <div class="form-group" id="code_zone">
          ...
          <div class="col-md-9">
            <input maxlength="<?php echo isset($code_len[$niveau-1])?$code_len[$niveau-1]:1; ?>" onblur="if(this.value!='' <?php if(isset($_GET["id"]) && !empty($_GET["id"])) echo "&& this.value!='".$row_liste_activite['code']."'"; ?>) check_code('verif_code.php?t=plan_budgetaire&','w=code='+this.value+' and niveau=<?php echo $niveau; ?> and projet=<?php echo $_SESSION["clp_projet"]; ?>','code_zone'); <?php if(isset($_GET["id"]) && !empty($_GET["id"])) echo "else $('#code_zone_text').html('&nbsp;');"; ?>" class="form-control required" type="text" name="code" id="code" value="<?php echo isset($row_liste_activite['code'])?$row_liste_activite['code']:""; ?>" size="32" />
    ...
    <tr valign="top">
      <td>
        <div class="form-group">
          ...
          <div class="col-md-9">
            <select style="width: 385px;" name="parent" id="parent" class="col-md-9 select2-select-00 required" data-placeholder="S&eacute;lectionnez <?php echo (isset($loc[$niveau-1]) && !empty($loc[$niveau-1]))?$loc[$niveau-1]:"Parent"; ?>" >
              <option></option>
