...
//exit;
 $query_liste_region = "SELECT distinct code_activite.annee FROM diffa.code_activite WHERE code!='Code' and code!='fichiers' group by annee order by annee asc";
  try{
...
$query_liste_periode = "SELECT sum(cout_realise) as cout, code_activite.annee FROM diffa.code_activite WHERE  code!='Code' and code!='fichiers' group by annee order by annee asc";
  try{
...
$titretext="D\u00e9caissement par ann\u00e9e (M F CFA) du ProDAF Diffa";
...
<div id="graph_diffa_fed" style="<?php if(isset($include_data)){ ?>min-width: 310px; height: 700px; margin: -5px auto;width:100%;<?php } else{ ?>height: 300px;  margin: 0 auto<?php } ?>"></div>
<script type="text/javascript">
Highcharts.chart('graph_diffa_fed', {
    chart: {
    ...
    legend: {
	 ...
	 tooltip: {
		 ...
		 credits: {
                enabled: true,
                href: 'http:#',
                text: 'PRODAF Diffa : <?php echo date("d/m/Y H:i"); ?>',
                style: {
                cursor: 'pointer',
                color: '#6633FF',
                fontSize: '10px',
