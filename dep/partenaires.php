...
if (isset($_GET["id_sup"])) {
  $id = $_GET["id_sup"];
  $insertSQL = sprintf("DELETE from ".$database_connect_prefix."acteur WHERE id_acteur=%s",
...
if ((isset($_POST["MM_form"])) && ($_POST["MM_form"] == "form1"))
{
  ...
  $personnel=$_SESSION['clp_id'];
    $insertSQL = sprintf("INSERT INTO ".$database_connect_prefix."acteur (map_partenaire, adresse_partenaire, type_partenaire, contact_partenaire, site_web, email_partenaire, description, code_acteur, nom_acteur, id_personnel) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, '$personnel')",
 ...
 try{
  ...
  $date=date("Y-m-d"); $personnel=$_SESSION['clp_id'];  $id = ($_POST["MM_update"]);
    $insertSQL = sprintf("UPDATE ".$database_connect_prefix."acteur SET map_partenaire=%s, adresse_partenaire=%s, type_partenaire=%s, contact_partenaire=%s, site_web=%s, email_partenaire=%s,  description=%s, code_acteur=%s, nom_acteur=%s, modifier_par='$personnel', modifier_le='$date' WHERE id_acteur=%s",
                        GetSQLValueString($_POST['map_partenaire'], "text"),
                        GetSQLValueString($_POST['adresse_partenaire'], "text"),
                        GetSQLValueString($_POST['type_partenaire'], "int"),
                        GetSQLValueString($_POST["contact_partenaire"], "text"),
...
<div class="widget-header"> <h4><i class="icon-reorder"></i> Acteurs du <?php print $config->structure; ?> </h4>
  <?php if(isset($_SESSION['clp_id']) && $_SESSION['clp_id']=='admin')
{
  echo do_link("","","Acteurs","<i class=\"icon-plus\"> Acteurs </i>","simple","./","pull-right p11","get_content('new_partenaire.php','','modal-body_add',this.title);",1,"",$nfile);
 // echo do_link("","","Secteur","<i class=\"icon-plus\"> Secteur </i>","simple","./","pull-right p11","get_content('new_sous_secteur.php','','modal-body_add',this.title);",1,"",$nfile);
}
?>
</div>
