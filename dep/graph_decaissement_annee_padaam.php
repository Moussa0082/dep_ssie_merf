...
//exit;
 $query_liste_region = "SELECT distinct code_activite.annee FROM mtz.code_activite, mtz.ptba WHERE code_activite.annee=ptba.annee and code!='Code' and code!='fichiers' group by annee order by annee asc";
  try{
...
$query_liste_periode = "SELECT sum(cout_realise) as cout, code_activite.annee FROM mtz.code_activite, mtz.ptba WHERE  code_activite.annee=ptba.annee  and code!='Code' and code!='fichiers' group by annee order by annee asc";
  try{
...
<div id="graph_padaam_fed" style="<?php if(isset($include_data)){ ?>min-width: 310px; height: 700px; margin: -5px auto;width:100%;<?php } else{ ?>height: 300px;  margin: 0 auto<?php } ?>"></div>
<script type="text/javascript">
Highcharts.chart('graph_padaam_fed', {
    chart: {
