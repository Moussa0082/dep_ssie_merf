...
echo '<div class="modal-header" style="height:auto; margin: 0; padding: 0">
               ...
               <center><h4 class="modal-title" id="note_fiche">Modification </h4></center>
          </div>
          <br>
...
$Table_Contenu = null;
$Table_Contenu=explode(';', $Contenu);

for ($jj=0; $jj < count($Table_Choix); $jj++) {$Table_Choix[$jj] = trim($Table_Choix[$jj]);}
for ($jj=0; $jj < count($Table_Contenu); $jj++) {$Table_Contenu[$jj] = trim($Table_Contenu[$jj]);}
//echo '<option value=""></option>';
for ($i=0; $i<count($Table_Choix); $i++)
  {

if(in_array($Table_Choix[$i], $Table_Contenu) != false)
  {echo '<option value="'.trim($Table_Choix[$i]).'" selected>'.trim($Table_Choix[$i]).'</option>';}
else {echo '<option value="'.trim($Table_Choix[$i]).'">'.trim($Table_Choix[$i]).'</option>';}
    }
    echo ' </select>
<input type="hidden" name="Colonne[]" value="'.$row8['Nom_Collone'].'">
<input type="hidden" name="Type[]" value="'.$row8['Type_Ligne'].'">
...
echo '<option value=""></option>';
for ($i=0; $i<count($Table_Choix); $i++)
  {if(trim($Table_Choix[$i])==trim($Contenu)){echo '<option value="'.trim($Table_Choix[$i]).'" selected>'.trim($Table_Choix[$i]).'</option>';}
else {echo '<option value="'.trim($Table_Choix[$i]).'">'.trim($Table_Choix[$i]).'</option>';}
    }
    echo ' </select>
<input type="hidden" name="Colonne[]" value="'.$row8['Nom_Collone'].'">
