 ...
 PC_Enregistrer_Code("DROP VIEW ".$row44["Nom_View"]);}}
  ...
  {foreach (FC_Rechercher_Code('SELECT * FROM t_feuille WHERE Code_Feuille='.$select_feuille) as $row4)
	...
	/*foreach (FC_Rechercher_Code('SELECT * FROM t_feuille_ligne WHERE (Code_Feuille='.$select_feuille." AND Nom_Ligne='".$input_regrouper_par."')") as $row6)
		...
		$Structure_View.=$input_regrouper_par." AS `".$input_regrouper_par."`, ";
...
/*foreach (FC_Rechercher_Code('SELECT * FROM t_feuille_ligne WHERE (Code_Feuille='.$select_feuille." AND Nom_Ligne='".$input_regrouper_par."')") as $row7)
		...
		$Structure_View.=$input_regrouper_par;

	PC_Enregistrer_Code($Structure_View);
	PC_Enregistrer_Code("INSERT INTO t_rapport(Nom_Rapport, Code_Feuille, Group_By, Valeur, Operation,  Nom_View, Structure_View, Type_Rapport, Feuille_Jointure, Attribut_Jointure_FP, Attribut_Jointure_FS, Id_Projet) VALUES ('".$nom_rapport."', $select_feuille, '".$input_regrouper_par."', '".$input_valeur."', '".$operation."', '".str_replace("t", "vw_r", $row4["Table_Feuille"])."_".$t."', '', 'SIMPLE', $FEUILLE_JOINTURE, '$attribut_jointure_fp', '$attribut_jointure_fs','".$_SESSION['clp_projet']."')");
$IND=0;
