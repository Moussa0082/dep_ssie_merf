...
echo '<div class="modal-header" style="height:auto; margin: 0; padding: 0">
               ...
               <center><h4 class="modal-title">Insertion des données </h4></center>
          </div>
          <br>
...
for ($i=0; $i<count($Table_Choix); $i++) {echo '<option value="'.$Table_Choix[$i].'">'.$Table_Choix[$i].'</option>';}
    echo ' </select>
<input type="hidden" name="Colonne[]" value="'.$row8['Nom_Collone'].'">
<input type="hidden" name="Type[]" value="'.$row8['Type_Ligne'].'">
  ';
...
foreach (FC_Rechercher_Code('SELECT * FROM '.str_replace("t", "v", $Nom_View)." WHERE Stat=1") AS $row10) 
