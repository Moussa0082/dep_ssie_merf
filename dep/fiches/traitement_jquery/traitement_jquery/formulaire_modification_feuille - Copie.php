...
{echo '<option value="Oui">Oui</option><option value="Non" selected>Non</option>';}
echo '</select></div><div class="col-sm-1 col-md-1 mb-1"></div><div class="col-sm-1 col-md-1 mb-1"><div class="btn btn-xs btn-default" title="Supprimer" ><span class="nav-label glyphicon glyphicon-remove text-danger" ></span></div></div></div><div class="row" id="Under_Div_'.$Compte.'">';
switch ($row5['Type_Ligne'])
{
  case 'CHOIX':
    echo '<div class="col-sm-2 col-md-2 mb-2"></div><div class="col-sm-6 col-md-6 mb-6"><input type="text" class="form-control" required placeholder="Sepaper les valeurs par point-virgule \';\' Exemple: HOMME ; FEMME"  onkeyup="document.getElementById(\'Valeur_'.$Compte.'\').value=this.value" value="';
...
echo '"></div><input type="hidden" value="'.$Valeur.'" name="valeur[]" id="Valeur_"'.$Compte.'>';    
    ...
    case 'FEUILLE':
    echo '<div class="col-sm-2 col-md-2 mb-2"></div><div class="col-sm-3 col-md-3 mb-3"><select name="" id="Col_Nom2_'.$Compte.'" onchange="Charger_Col_Nom(this.value, \'Col_Nom_'.$Compte.'\')" id="" required class="form-control"><option value="0">Feuille</option>';
  $Valeur="";
  foreach (FC_Rechercher_Code('SELECT * FROM t_feuille INNER JOIN t_feuille_etrangere ON (t_feuille.Table_Feuille=t_feuille_etrangere.Nom_Table) WHERE (t_feuille_etrangere.Nom_Table=\''.$row4['Table_Feuille'].'\' AND t_feuille_etrangere.Nom_Colonne=\''.$row5['Nom_Collone'].'\')') as $row8) 
{$Valeur= $row8['Valeur'];}
$Table_Choix=null;
