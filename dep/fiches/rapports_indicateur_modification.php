...
require_once 'theme_components/theme_style.php';
 ...
 $Code_Rapport="";
      ...
      {$Code_Rapport=base64_decode($_GET['r']);
        ...
        $Code_Classeur2="";
        $ii=0;
        foreach (FC_Rechercher_Code('SELECT * FROM t_rapport_indicateur WHERE Code_Rapport=\''.$Code_Rapport.'\'') as $row4)
        {$ii++; $uuu=0;
...
<form id="form_rapport_dynamique" action="traitement_jquery\inserer_rapport_indicateur.php" method="POST">
  <input type="hidden" name="Code_Rapport" id="Code_Rapport" <?php echo 'value="'.base64_encode($row4["Code_Rapport"]).'"'; ?>>
   ...
   <div class="row" style="">
      <div class="col-md-4 col-lg-4">
        ...
        <select class="selectpicker bts_select form-control" data-live-search="true" name="nom_rapport" id="nom_rapport">

     <?php 
        $Exp_GROUP_BY = explode(".", $row4["Group_By"]);
    ...
    foreach (FC_Rechercher_Code('SELECT t_feuille_ligne.*, Table_Feuille FROM `t_feuille_ligne` INNER JOIN `t_feuille` ON (t_feuille_ligne.Code_Feuille = t_feuille.Code_Feuille) WHERE t_feuille_ligne.Code_Feuille='.$row4["Code_Feuille"]) as $row7){
     ...
     foreach (FC_Rechercher_Code("SELECT `code_ref_ind` AS code, `intitule_ref_ind` AS intitule FROM referentiel_indicateur  WHERE (code_ref_ind='".$row4["Indicateur"]."')") as $row3) 
      {echo '<option value="'.$row3["code"].'">'.$row3["intitule"].'</option>';}?>
        
      <?php foreach (FC_Rechercher_Code("SELECT `code_ref_ind` AS code, `intitule_ref_ind` AS intitule FROM referentiel_indicateur WHERE (code_ref_ind NOT IN (SELECT Indicateur FROM t_rapport_indicateur))") as $row3) 
      {echo '<option value="'.$row3["code"].'">'.$row3["intitule"].'</option>';}
       ?>  
        </select>
      </div> 
      ...
      <div class="col-md-4 col-lg-4">
        <label style="float: left;">Affichage sur la page d'accueil</label>
        <select class="form-control" name="affichage" id="affichage">
     <?php 
        $AFFICHAGE_TAB_V[]="Membre du projet";$AFFICHAGE_TAB_V[]="UN";$AFFICHAGE_TAB_V[]="Tous";

        for($i=0; $i<count($AFFICHAGE_TAB_V); $i++) {
          if($AFFICHAGE_TAB_V[$i]==$row4["Affichage"]){echo '<option value="'.$AFFICHAGE_TAB_V[$i].'" selected>'.$AFFICHAGE_TAB_V[$i].'</option>';}
          else{echo '<option value="'.$AFFICHAGE_TAB_V[$i].'">'.$AFFICHAGE_TAB_V[$i].'</option>';}
        }
       ?>
      ...
      <div class="col-md-4 col-lg-4"><br>
         <button style="width: 150px" type="button" id="Boutton_Ajouter_Critere" <?php echo 'class="btn '.$Boutton_Style.'"'; ?>>Ajouter un critère</button>
      </div>
   </div><br>
   <div class="row" style="">
 ...
 ?>
  ...
  <?php 
  /*      $ind=0;
      foreach (FC_Rechercher_Code('SELECT * FROM t_feuille_ligne WHERE Code_Feuille='.$row4["Feuille_Jointure"]) as $row7){
     if($row4["Attribut_Jointure_FS"]==$row7["Nom_Ligne"])
     {echo '<option value="'.$row7["Nom_Ligne"].'" class="option_champ_'.str_replace(" ", "", $row7["Nom_Ligne"]).'" id="" selected>'.$row7["Nom_Ligne"].'</option>';}
   else {echo '<option value="'.$row7["Nom_Ligne"].'" class="option_champ_'.str_replace(" ", "", $row7["Nom_Ligne"]).'" id="">'.$row7["Nom_Ligne"].'</option>';}
      $ind++;
   }*/
 ?>
 ...
 ?>
      ...
      <div class="col-md-4 col-lg-4">
        <label style="float: left;">Reprouper par</label>
         <select name="input_regrouper_par[]"  multiple id="input_regrouper_par" class="form-control selectpicker bts_select" data-live-search="true">
...
<option value=""></option>
        ...
        if(in_array("ANNEE", explode(';', $row4["Group_By"])))
        {
      ...
      echo '<option value="'.$Liste_1_Col_Val[$i].'" class="option_champ_'.str_replace(" ", "", $Liste_1_Col_Lab[$i]).'" id=""';
       if(in_array($Liste_1_Col_Val[$i], explode(';', $row4["Group_By"]))){echo " selected ";}
...
if(!empty($row4["Feuille_Jointure"])){
      ...
      echo '<option value="'.$Liste_2_Col_Val[$i].'" class="option_champ_'.str_replace(" ", "", $Liste_2_Col_Lab[$i]).'" id=""';
       if($row4["Group_By"] == $Liste_2_Col_Val[$i]){echo " selected ";}
      echo '>'.$Liste_2_Col_Lab[$i].'</option>';}
    echo '</optgroup>';
    }
          echo '<optgroup label="Année"><option value="ANNEE" selected>ANNEE</option></optgroup>';
    ...
    for($i=0; $i<count($Liste_1_Col_Val); $i++){
      echo '<option value="'.$Liste_1_Col_Val[$i].'" class="option_champ_'.str_replace(" ", "", $Liste_1_Col_Lab[$i]).'" id=""';
       if($row4["Group_By"] == $Liste_1_Col_Val[$i]){echo " selected ";}
      echo '>'.$Liste_1_Col_Lab[$i].'</option>';}
    echo '</optgroup>';
...
if(!empty($row4["Feuille_Jointure"])){
  ...
  echo '<optgroup label="Année"><option value="ANNEE">ANNEE</option></optgroup>';
   }
...
if(!empty($row4["Feuille_Jointure"])){
    ...
    echo '</optgroup>';
      ...
      <option value=""></option>
     <?php 
        $OPERATION_TAB_V[]="COUNT";$OPERATION_TAB_V[]="SUM";$OPERATION_TAB_V[]="AVG";$OPERATION_TAB_V[]="MIN";$OPERATION_TAB_V[]="MAX";
        $OPERATION_TAB_L[]="COMPTER";$OPERATION_TAB_L[]="SOMME";$OPERATION_TAB_L[]="MOYENNE";$OPERATION_TAB_L[]="MIN";$OPERATION_TAB_L[]="MAX";
...
var COLS_FP="";
COLS_FP=document.getElementById("input_valeur").innerHTML;
var COLS_FS="";
...
COLS_FP = data;
     ...
     $("#input_regrouper_par").html(COLS_FP + COLS_FS.replace('<option value=""></option>','')+'<optgroup label="Année"><option value="ANNEE">ANNEE</option></optgroup>'); 
     $('.selectpicker').selectpicker('refresh');
     $("#input_valeur").html(COLS_FP + COLS_FS.replace('<option value=""></option>',''));
     $("#attribut_jointure_fp").html(COLS_FP);  

      /*$("#input_regrouper_par").html(data+'<option value="ANNEE">ANNEE</option>'); 
      $("#input_valeur").html(data);  
      $("#attribut_jointure_fp").html(data); */
...
COLS_FS = data;
     ...
     $("#attribut_jointure_fs").html(COLS_FS);
      //$("#attribut_jointure_fs").html(data); 
...
if(document.getElementById("nom_rapport").value=="" || document.getElementById("select_feuille").value=="" || document.getElementById("input_valeur").value=="" || document.getElementById("operation").value=="")
...
/*for(i=0; i<champ_criteres.length; i++)
  ...
  $.ajax({url:"traitement_jquery/inserer_rapport_indicateur.php?action=modif", method:"POST", data:$('#form_rapport_dynamique').serialize(), success:function (data) {
...
<?php }
if($ii==0){header('location:rapports_indicateur.php');}
    }
      else
