...
$query_liste_groupes_travail = "SELECT * FROM partenaire where code in (select bailleur from diffa.type_part) order by code";
 try{
...
$query_liste_periode = "SELECT sum(cout_realise) as cout, sigle FROM diffa.code_convention, diffa.type_part, pnf.partenaire WHERE code_convention.code=type_part.code_type and type_part.bailleur=partenaire.code and code_convention.projet=type_part.projet  group by  sigle order by sigle desc";	
 try{
...
$query_liste_type_reunion = "SELECT sum(montant) as cout, sigle, count(distinct type_part.projet) as nb_projet  FROM pnf.partenaire, diffa.type_part WHERE code=bailleur  GROUP BY sigle";
 try{
...
<thead>
<tr role="row">
  <th rowspan="2" data-class="expand">  <div align="left">Bailleurs du <br/> Projet ProDAF Diffa</div></th>
