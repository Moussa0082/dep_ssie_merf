 ...
 *   - the class can now output the uploaded files content as the return value of process() if the function is called with an Aucun or null argumenti, or no argument</li>
 ...
 *   - if MIME is Aucun, the class now triggers an error<br>
...
class upload {
     ...
     *   and if it is Aucun, will default to the system's default
     * If set to an Aucun string, it will call finfo_open without the path argument
     *
     * This variable is set to true by default for security reason
     *
     * @access public
                ...
                if ($this->mime_getimagesize) {
                    ...
                    if (is_array($info) && array_key_exists('mime', $info)) {
                        ...
                        if (empty($this->file_src_mime)) {
                            $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;MIME Aucun, guessing from type<br />';
    ...
    function getsize($size) {
        $size = (int) $size; $last = strtolower($size{strlen($size)-1});
        switch($last) {
            case 'g':
                $size *= 1024;
            case 'm':
     ...
     *
     * If the function is called with a null or Aucun argument, then it will return the content of the picture
     *
