...
if ((isset($_POST["MM_form"])) && ($_POST["MM_form"] == "form0"))
{
  if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "MM_insert"))
  {
    ...
    if(in_array($ext,$extensions_autorisees))
    {
      ...
      elseif(isset($_FILES['fichier']) && $_FILES['fichier']['error'] == 0)
      {
        ...
        $query_sup_import_annee = sprintf("DELETE from ".$database_connect_prefix."cadre_cosop WHERE niveau=%s",
  ...
  try{
        ...
        for ($row = 5; $row <= $highestRow; $row++)
        {
            ...
            {
              $insertSQL = sprintf("INSERT INTO ".$database_connect_prefix."cadre_cosop (intitule,niveau,code,parent, id_personnel) VALUES (%s, %s, %s, %s, '$personnel')",
...
if ((isset($_POST["MM_form"])) && ($_POST["MM_form"] == "form1"))
{
  ...
  if ((isset($_POST["MM_update"]) && !empty($_POST["MM_update"]))) {
    $date=date("Y-m-d"); $personnel=$_SESSION['clp_id']; $c=$_POST['id'];
  	...
  	$insertSQL = sprintf("UPDATE ".$database_connect_prefix."cadre_cosop SET intitule=%s, niveau=%s, code=%s, parent=%s, modifier_par='$personnel', modifier_le='$date' WHERE code='$c'",
                         GetSQLValueString($_POST['intitule'], "text"),
                         GetSQLValueString($_POST['niveau'], "int"),
  					     GetSQLValueString($_POST['code'], "text"),
                         GetSQLValueString($_POST['parent'], "text"));
...
if ((isset($_POST["MM_form"])) && ($_POST["MM_form"] == "form2"))
{
  ...
  $form= $_POST['form']; $noms= $form['lib']; $nombre=count($noms); $types= $form['indic'];
    ...
    $insertSQL = sprintf("INSERT INTO ".$database_connect_prefix."cadre_config_cosop (nombre, libelle, type) VALUES (%s, %s, %s)",
  ...
  $form= $_POST['form']; $noms= $form['lib']; $nombre=count($noms); $types= $form['indic'];
    ...
    $query_liste_actpa = sprintf("SELECT nombre FROM ".$database_connect_prefix."cadre_config_cosop");
	      	try{
    ...
    if($totalRows_liste_actpa>0 && $row_liste_actpa["nombre"]==$nombre+1){
      $insertSQL = sprintf("DELETE from ".$database_connect_prefix."cadre_cosop WHERE niveau=%s",
  ...
  try{
  	...
  	$insertSQL = sprintf("UPDATE ".$database_connect_prefix."cadre_config_cosop SET nombre=%s, libelle=%s, type=%s",
  					   GetSQLValueString($nombre, "int"),
  					   GetSQLValueString(implode(",",$noms), "text"),
                       GetSQLValueString(implode(",",$types), "text"));
...
<div class="widget-header"> <h4><i class="icon-reorder"></i> Cadre Analytique </h4>
<?php
...
if(isset($niveau)){ $lib = $libelle[$niveau]; if($niveau<count($libelle)) {
  ...
  echo do_link("new_cadre_niveau_cosop","","Ajout $lib","<i class=\"icon-plus\" btn-warning> Ajouter $lib </i>","","./","pull-right p11","get_content('new_cadre_cosop.php','niveau=".($niveau+1)."','modal-body_add',this.title);",1,"",$nfile);
