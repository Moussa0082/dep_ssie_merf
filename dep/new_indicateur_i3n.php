...
{
  $id=($_GET["id"]);
 // $query_liste_indicateur = "SELECT * FROM ".$database_connect_prefix."indicateur_cadre_resultat WHERE code_indicateur_i3n='$id'  and structure=%s and projet=%s and code_cr=%s and niveau=%s";
    $query_liste_indicateur = "SELECT * FROM ".$database_connect_prefix."indicateur_i3n WHERE code_indicateur_i3n='$id'";
          try{
...
<div class="widget-content">
  ...
  <form action="indicateur_i3n.php" class="form-horizontal row-border" method="post" enctype="multipart/form-data" name="form1" id="form1" novalidate="novalidate">
  <table border="0" id="mtable" align="center" cellspacing="1" cellpadding="0" width="100%" style="font-size:14px;">
    <tr valign="top">
      <td>
        <div class="form-group">
          <label for="code_indicateur_i3n" class="col-md-3 control-label">Code <span class="required">*</span></label>
          <div class="col-md-9">
            <input class="form-control required" type="text" name="code_indicateur_i3n" id="code_indicateur_i3n" value="<?php if(isset($_GET["id"]) && !empty($_GET["id"])) echo $row_liste_indicateur['code_indicateur_i3n']; ?>" size="32" onblur="if(this.value!='') check_code('verif_code.php?t=indicateur_i3n&','w=code_indicateur_i3n='+this.value+' and niveau=<?php echo $niveau; ?> ','code_zone');" />
    ...
    <tr valign="top">
      <td>
        <div class="form-group">
          <label for="intitule_indicateur_i3n" class="col-md-3 control-label">Intitul&eacute; <span class="required">*</span></label>
          <div class="col-md-9">
            <textarea class="form-control required" cols="200" rows="2" type="text" name="intitule_indicateur_i3n" id="intitule_indicateur_i3n"><?php if(isset($_GET["id"]) && !empty($_GET["id"])) echo $row_liste_indicateur['intitule_indicateur_i3n']; ?></textarea>
          </div>
        </div>
      </td>
    </tr>
    <tr valign="top">
      <td>
        <div class="form-group">
          <label for="produit_i3n" class="col-md-3 control-label"><?php echo (isset($libelle1[count($libelle1)-1]))?$libelle1[count($libelle1)-1]:"ND"; ?> <span class="required">*</span></label>
          <div class="col-md-9">
            <select name="produit_i3n" id="produit_i3n" class="form-control required" >
              <option value="">Selectionnez</option>
              <?php if($totalRows_liste_activite_1>0) { foreach($row_liste_activite_1 as $row_liste_activite_1){  ?>
              <option value="<?php echo $row_liste_activite_1['code']; ?>" <?php if (isset($_GET["id"]) && !empty($_GET["id"]) && $row_liste_activite_1['code']==$row_liste_indicateur['produit_i3n']) {echo "SELECTED";} ?>><?php echo $row_liste_activite_1['code'].": ".$row_liste_activite_1['intitule']; ?></option>
              <?php } } ?>
            </select>
          </div>
        </div>
