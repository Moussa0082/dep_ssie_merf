...
if (isset($_POST["MM_delete"]) && !empty($_POST["MM_delete"])) {
  ...
  header(sprintf("Location: %s", $insertGoTo)); exit();
      ...
      // ... code d'insertion ici ...
      if ((isset($_POST["MM_form"])) && ($_POST["MM_form"] == "form1")) {
        if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "MM_insert")) {
            $date = date("Y-m-d");
            $personnel = $_SESSION['clp_id'];
    
            $insertSQL = "INSERT INTO " . $database_connect_prefix . "indicateur_i3n (produit_i3n, niveau, code_indicateur_i3n, intitule_indicateur_i3n, referentiel, description, date_enregistrement, id_personnel) VALUES (:produit_i3n, :niveau, :code_indicateur_i3n, :intitule_indicateur_i3n, :referentiel, :description, '$date', '$personnel')";
    
            try {
                $stmt = $pdar_connexion->prepare($insertSQL);
                $stmt->bindParam(':produit_i3n', $_POST['produit_i3n'], PDO::PARAM_STR);
                $stmt->bindParam(':niveau', $niveau+1, PDO::PARAM_INT);
                $stmt->bindParam(':code_indicateur_i3n', $_POST['code_indicateur_i3n'], PDO::PARAM_STR);
                $stmt->bindParam(':intitule_indicateur_i3n', $_POST['intitule_indicateur_i3n'], PDO::PARAM_STR);
...
if (isset($_POST['MM_update']) && !empty($_POST['MM_update'])) {
  ...
  $updateSQL = "UPDATE " . $database_connect_prefix . "indicateur_i3n SET 
                  produit_i3n = :produit_i3n,
                  niveau = :niveau,
                  code_indicateur_i3n = :code_indicateur_i3n,
                  intitule_indicateur_i3n = :intitule_indicateur_i3n,
                  referentiel = :referentiel,
                  description = :description,
                  modifier_par = :modifier_par,
                  modifier_le = :modifier_le
                  WHERE code_indicateur_i3n = :code";
