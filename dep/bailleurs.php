...
//Bailleurs
$query_liste_bailleur = "SELECT * FROM ".$database_connect_prefix."partenaire where code in (select bailleur from ".$database_connect_prefix."type_part)";
try{
    $liste_bailleur = $pdar_connexion->prepare($query_liste_bailleur);
    $liste_bailleur->execute();
...
$query_liste_part = "SELECT id_partenaire, count(bailleur) as nbbail, sum(montant) as partb FROM ".$database_connect_prefix."partenaire, ".$database_connect_prefix."type_part WHERE code=bailleur  GROUP BY id_partenaire";
try{
