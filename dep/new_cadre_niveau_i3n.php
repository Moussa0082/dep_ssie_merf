...
$date=date("Y-m-d");
  ...
  $query_liste_activite = "SELECT * FROM ".$database_connect_prefix."cadre_config_i3n  LIMIT 1";
    try{
...
<script>
 ...
 var index= 0; var nbrch = <?php echo count($libelle); ?>;
	   ...
	   function add_input($container,num,lib_v,nodel,niv,t){
	    	       var $prototype=$(' <tr><td><label for="form_lib_'+index+'">'+'Niveau '+(index+1)+'&nbsp;</label>'+'<input placeholder="Libell�" required="required" type="text" id="form_lib_'+index+'" size="30" name="form[lib]['+index+']" value="'+((lib_v)?lib_v:'')+'" class="form-control" /><input type="hidden" id="form_niveau_'+index+'" size="30" name="form[niveau]['+index+']" value="'+(index+1)+'" /></td><td>');
...
<form action="niveau_i3n.php" class="form-horizontal row-border" method="post" enctype="multipart/form-data" name="form2" id="form2" novalidate="novalidate">
