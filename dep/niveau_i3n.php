...
if (isset($_POST["MM_form"]) && $_POST["MM_form"] == "form1") {
  ...
  if (isset($_POST["MM_insert"]) && $_POST["MM_insert"] == "MM_insert") {
      ...
      try {
          ...
          $insertGoTo .= $stmt ? "?insert=ok&add_new=1" : "?insert=no";
  ...
  if (isset($_POST["MM_delete"]) && !empty($_POST["MM_delete"])) {
      $id = $_POST["MM_delete"];
      $deleteSQL = "DELETE FROM " . $database_connect_prefix . "cadre_config_i3n WHERE id = :id";
  ...
  if (isset($_POST["MM_update"]) && !empty($_POST["MM_update"])) {
      $id = $_POST['id'];
      $updateSQL = "UPDATE " . $database_connect_prefix . "cadre_config_i3n SET nombre = :nombre, libelle = :libelle, budget = :budget, type = :type, structure = :structure, projet = :projet WHERE id = :id";
...
//     $insertSQL = sprintf("INSERT INTO ".$database_connect_prefix."cadre_config_i3n (nombre, libelle, budget, type) VALUES (%s, %s, %s, %s)",
...
//   	$insertSQL = sprintf("UPDATE ".$database_connect_prefix."cadre_config_i3n SET nombre=%s, libelle=%s, budget=%s, type=%s",
//   					   GetSQLValueString($nombre, "int"),
//   					   GetSQLValueString(implode(",",$noms), "text"),
//                        GetSQLValueString(implode(",",$types), "text"));
...
// }
 ...
 // Début du traitement du formulaire form2
if (isset($_POST["MM_form"]) && $_POST["MM_form"] == "form2") {
  // Insertion
  if (isset($_POST["MM_insert"]) && $_POST["MM_insert"] == "MM_insert") {
      handleFormInsert();
...
function handleFormInsert() {
  global $database_connect_prefix, $pdar_connexion;
...
function handleFormDelete() {
  ...
  $nombre = count($_POST['form']['lib']) + 1;

  $deleteSQL = "DELETE FROM " . $database_connect_prefix . "cadre_i3n WHERE niveau=:niveau";

  try {
      $stmt = $pdar_connexion->prepare($deleteSQL);
      $stmt->bindParam(':niveau', $nombre, PDO::PARAM_INT);
      $stmt->execute();
  } catch (Exception $e) {
      die(mysql_error_show_message($e));
  }
...
function redirectToPage($stmt, $queryParam) {
  $insertGoTo = $_SERVER['PHP_SELF'] . "?niveau=" . intval($_GET["niveau"]) . $queryParam;
  header("Location: $insertGoTo");
  exit();
}
