...
{ //Fonction
  if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "MM_insert")) {
  $insertSQL = sprintf("INSERT INTO ".$database_connect_prefix."groupe_etape (code_groupe, categorie_groupe, libelle_groupe, id_personnel, date_enregistrement) VALUES (%s, %s, %s,'$personnel', '$date')",
  ...
  if (isset($_POST["MM_update"]) && !empty($_POST["MM_update"])) {
    $id = ($_POST["MM_update"]);
  	$insertSQL = sprintf("UPDATE ".$database_connect_prefix."groupe_etape SET code_groupe=%s, categorie_groupe=%s, libelle_groupe=%s, etat='Modifié', modifier_par='$personnel', modifier_le='$date' WHERE id_groupe='$id'",
					   GetSQLValueString($_POST['code_groupe'], "text"),
					   GetSQLValueString($_POST['categorie'], "text"),
					   GetSQLValueString($_POST['libelle_groupe'], "text")//,
					   );
...
//Etatpes
$query_liste_g_etape = "SELECT * FROM ".$database_connect_prefix."groupe_etape  ORDER BY code_groupe asc";
try{
...
<div class="col-md-6">
<div class="widget box ">
 <div class="widget-header"> <h4><i class="icon-reorder"></i> Types ou Cat&eacute;gories de march&eacute;s</h4>
...
<?php
echo do_link("","","Edition des cat&eacute;gories de march&eacute;s","<i class=\"icon-plus\"> Ajouter </i>","","./","pull-right p11","get_content('new_categorie_marche.php','','modal-body_add',this.title);",1,"",$nfile);
?>
...
<div class="widget-content scroller">
 <table border="0" cellspacing="3" class="table table-striped table-bordered table-hover table-responsive dataTable hide_befor_load" align="center" >
      ...
      <tr>
        ...
        <?php if(isset($_SESSION['clp_niveau']) && ($_SESSION['clp_niveau']==1)) { ?>
        <td align="center"><?php
echo do_link("","","Modifier une cat&eacute;gorie de march&eacute;s","","edit","./","","get_content('new_categorie_marche.php','id=$id','modal-body_add',this.title);",1,"margin:0px 5px;",$nfile);

echo do_link("",$_SERVER['PHP_SELF']."?id_sup=".$id,"Supprimer","","del","./","","return confirm('Voulez-vous vraiment supprimer cette cat&eacute;gorie ?');",0,"margin:0px 5px;",$nfile);
?>          </td>
        <?php } ?>
      </tr>
      <?php } } ?>
...
<div class="widget-header">
 <h4><i class="icon-reorder"></i> Types ou M&eacute;thodes de passation</h4>
 <div class="toolbar no-padding"><div class="btn-group"><span class="btn btn-xs widget-collapse pull-right"><i class="icon-angle-down"></i></span>
  <?php if(isset($_SESSION['clp_niveau']) && $_SESSION['clp_niveau']==1){ ?>
<?php
...
<table border="0" cellspacing="3" class="table table-striped table-bordered table-hover table-responsive dataTable hide_befor_load" align="center" >
      <thead>
        <tr>
          <th><div align="left"><strong>Cat&eacute;gorie</strong></div></th>
