...
$query_liste_taux_tache1 = "SELECT count(id_groupe_tache) as nb_tache, sum(if(statut!='auto',1,0)) as tache_annule,  left(code_activite_ptba,1) as cp, ugl FROM ptba, groupe_tache, tache_ugl WHERE id_ptba=groupe_tache.id_activite and id_groupe_tache=tache and ptba.annee='$annee' $wheract_tache $wheract  and ptba.projet='".$_SESSION["clp_projet"]."' group by cp, ugl";
...
$query_liste_taux_tache_act = "SELECT id_ptba, left(code_activite_ptba,1) as cp, tsuivi, lot_cible, cible.id_groupe_tache, cible.ugl FROM 
...
$query_liste_taux_activite_cp = "select  total, left(code_activite_ptba,1) as cp from (SELECT SUM(if(n_lot>0 && valider=1, proportion*jalon/n_lot,0)) as total, id_ptba, code_activite_ptba FROM ptba inner join  groupe_tache ON ptba.id_ptba = groupe_tache.id_activite where ptba.annee='$annee' $wheract and ptba.projet='".$_SESSION["clp_projet"]."' GROUP BY id_ptba, code_activite_ptba) as r1";
...
<form name="form<?php echo $annee; ?>" id="form<?php //echo $annee; ?>" method="get" action="<?php echo "recap_taches_activite_ptba.php?annee=".$annee; ?>" class="pull-left">
