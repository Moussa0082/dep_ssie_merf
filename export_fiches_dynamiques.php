...
header("content-disposition: attachment;filename=".((isset($_GET["nom"])?$_GET["nom"].date("d_m_Y"):"fiche_".date("d_m_Y"))).".xls");
...
$query_act = "SELECT * FROM ".$database_connect_prefix."$cp WHERE annee=$annee and projet='".$_SESSION["clp_projet"]."' and structure like '".$cmp."' "; //='".$_SESSION["clp_struture"]."'
...
mysql_select_db($database_pdar_connexion, $pdar_connexion);
$query_entete = "DESCRIBE ".$database_connect_prefix."$cp";
$entete  = mysql_query($query_entete , $pdar_connexion) or die(mysql_error());
$row_entete  = mysql_fetch_assoc($entete);
$totalRows_entete  = mysql_num_rows($entete);
$num=0;
...
<?php }  do{  if($row_entete["Field"]!="LKEY" && in_array($row_entete["Field"],$entete_array)){
...
{ foreach($choix_array as $Col=>$Val){ $chx[$Col]=explode(";",$Val);
 ...
 ?>
<td class=" "><?php if(strtolower($row_entete["Type"])=="date") echo implode('-',array_reverse(explode('-',$row_act[$row_entete["Field"]])));
...
<?php  }elseif($i==0) $row_entete  = mysql_fetch_assoc($entete); $i++; }while($row_entete  = mysql_fetch_assoc($entete));
$rows = mysql_num_rows($entete);
if($rows > 0) {
mysql_data_seek($entete, 0);
$row_entete = mysql_fetch_assoc($entete);
