...
<div class="tabbable tabbable-custom" >
  <ul class="nav nav-tabs" >
  <?php for($j=$_SESSION["annee_debut_projet"];$j<=date("Y");$j++){ if($j<=$_SESSION["annee_fin_projet"]) {?>
    <li title="" class="<?php echo ($j==$annee)?"active":""; ?>"><a href="#tab_global_<?php echo $j; ?>" data-toggle="tab"><?php echo $j; ?></a></li>
  <?php }} ?>
  </ul>
  <div class="tab-content">
  <?php for($j=$_SESSION["annee_debut_projet"];$j<=date("Y");$j++){ if($j<=$_SESSION["annee_fin_projet"]) {?>
  <div class="tab-pane <?php echo ($j==$annee)?"active":""; ?>" id="tab_global_<?php echo $j; ?>" data-target="./index_execution_ptba_consolide.php?annee=<?php echo $j; ?>" >
  </div>
  <?php }} ?>
  </div>
</div></div>
