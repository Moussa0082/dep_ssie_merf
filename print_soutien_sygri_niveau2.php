...
header("content-disposition: attachment;filename=Soutien_SYGRI_Niveau2.xls"); }
else if(isset($_GET["down"]) && isset($_GET["t"]) && $_GET["t"]=="w"){
header("Content-Type: application/vnd.ms-word");
header("Expires: 0");
header("Cache-Control: must-revalidate, post-check=0, pre-check=0");
header("content-disposition: attachment;filename=Soutien_SYGRI_Niveau2.rtf"); }
...
$query_scomposante = "SELECT id_sous_composante	, code_sous_composante, intitule_sous_composante FROM sous_composante, composante where id_composante=composante order by code_sous_composante";
...
<br />
<?php } ?>
<h4 align="center">Liste des indicateurs de soutien au 2&egrave;me Niveau de SYGRI  par sous/composantes du FIER Editer le <?php echo date("d/m/Y"); ?></h4>


<table width="60%" border="1" align="center" cellspacing="0">
  <tr class="titrecorps2">
        <td align="center" width="70%">Intitul&eacute; de l'indicateur </td>
        <td align="center" width="10%">&nbsp;Unit&eacute;&nbsp;</td>
        <td align="center" width="10%">&nbsp;Cible&nbsp;</td>
        <td align="center" width="10%">&nbsp;Proportion&nbsp;</td>
      </tr>
  <?php /*if($totalRows_scomposante>0) {$o2=0;do {*/ ?>
   <?php 
				    $id_scp=$row_scomposante['id_sous_composante'];
				    mysql_select_db($database_pdar_connexion, $pdar_connexion);
					$query_ind = "SELECT * FROM sous_composante,composante, indicateur_sygri2_projet LEFT JOIN soutien_indicateur_sygri2 ON indicateur_sygri_niveau2=id_indicateur_sygri_niveau2_projet where sous_composante=id_sous_composante and composante=id_composante order by code_sous_composante";
  ...
  <?php if($totalRows_ind>0) { ?>
      <?php $i=0; $p1="j"; $p11="j"; do { ?>
      <?php  if($p11!=$row_ind['code_sous_composante']) {?>
      <tr <?php  echo 'bgcolor="#D2E2B1"'; ?>>
        <td align="center" colspan="4"><div align="left"><b>
          <?php  if($p1!=$row_ind['code_sous_composante']) { echo $row_ind['code_sous_composante'].": ".$row_ind['intitule_sous_composante']; $i=0; }$p11=$row_ind['code_sous_composante']; ?>
        </b></div></td>
      </tr>
      <?php } ?>
      <?php  if($p1!=$row_ind['id_indicateur_sygri_niveau2_projet']) {?>
      <tr <?php  echo 'bgcolor="#D2E2B1"'; ?>>
        <td align="center" colspan="4"><div align="left">
          <?php  if($p1!=$row_ind['id_indicateur_sygri_niveau2_projet']) {echo $row_ind['intitule_indicateur_sygri2']; $i=0; }$p1=$row_ind['id_indicateur_sygri_niveau2_projet']; ?>
        </div></td>
      </tr>
      <?php } ?>
      <tr <?php if($i%2==0) echo 'bgcolor="#F9F9F7"';  $i=$i+1;?> onmouseover="this.bgColor='#CCFFFF';" onmouseout="this.bgColor='<?php if($i%2!=0) echo '#F9F9F7'; ?>';">
        <td width="70%"><div align="left" class="Style14"><?php echo $row_ind['intitule_indicateur_soutien']; ?></div></td>
        <td width="10%"><div align="center"><?php if(isset($unite_ind_ref_array[$row_ind["referentiel"]])) echo " (".$unite_ind_ref_array[$row_ind["referentiel"]].")"; ?></div></td>
        <td width="10%"><div align="center"><span class="Style49"><?php echo $row_ind['cible']; ?></span></div></td>
        <td width="10%"><div align="center"><span class="Style14"><?php echo $row_ind['proportion']; ?>%</span></div></td>
      </tr>
