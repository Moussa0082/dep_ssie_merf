...
if(isset($_SESSION["clp_projet"])  && !empty($_SESSION["clp_projet"])){
$query_liste_cout = "SELECT SUM( if(cout_realise>0, cout_realise,0) ) as realise  FROM ".$database_connect_prefix."code_activite where ".$database_connect_prefix."code_activite.projet='".$_SESSION["clp_projet"]."'  and  code!='Code' and code!='fichiers' and annee in (select id_version_ptba FROM ".$database_connect_prefix."version_ptba)";
try{
    $liste_cout = $pdar_connexion->prepare($query_liste_cout);
    $liste_cout->execute();
    $row_liste_cout = $liste_cout ->fetch();
...
$query_liste_cout = "SELECT SUM( if(montant>0, montant,0) ) as prevu FROM ".$database_connect_prefix."type_part, partenaire where code=bailleur and  ".$database_connect_prefix."type_part.projet='".$_SESSION["clp_projet"]."'";
try{
    $liste_cout = $pdar_connexion->prepare($query_liste_cout);
    $liste_cout->execute();
    $row_liste_cout = $liste_cout ->fetch();
...
<li class="dropdown user"> <a href="#" class="dropdown-toggle" data-toggle="dropdown" style="padding-top: 4px;padding-bottom: 4px;text-align: center;line-height: 21px;"> <i class="icon-male"></i> <span class="username"><?php echo $_SESSION["clp_nom"]."<br>".$_SESSION["clp_prenom"]; ?></span> <i class="icon-caret-down small"></i> </a>
  <ul class="dropdown-menu">
    ...
    <li><a class="<?php echo (isset($nfile) && $nfile == "my_agenda.php")?'current':''; ?>" href="./my_agenda.php"><i class="icon-folder"></i> Mon agenda</a></li>
  ...
  <!--   <li><a class="<?php echo (isset($nfile) && $nfile == "mise_a_jour.php")?'current':''; ?>" href="./mise_a_jour.php"><i class="icon-refresh"></i> Mise à jour <span class="badge label-success" id="notif_update_num"></span></a></li>
   <li><a class="<?php echo (isset($nfile) && $nfile == "courrier_dno.php")?'current':''; ?>" href="./courrier_dno.php"><i class="icon-envelope"></i> Courrier DANO</a></li>  -->
    <li class="divider"></li>
<!--    <li><a class="<?php echo (isset($nfile) && $nfile == "structures.php")?'current':''; ?>" href="./structures.php"><i class="icon-cogs"></i> Configuration système</a></li> -->
