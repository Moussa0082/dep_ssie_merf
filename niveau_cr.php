...
if ((isset($_POST["MM_form"])) && ($_POST["MM_form"] == "form0"))
{
  if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "MM_insert"))
  {
    ...
    if(in_array($ext,$extensions_autorisees))
    {
      ...
      elseif(isset($_FILES['fichier']) && $_FILES['fichier']['error'] == 0)
      {
        ...
        $query_sup_import_annee = sprintf("DELETE from ".$database_connect_prefix."cadre_logique WHERE niveau=%s and projet=%s",
  ...
  try{
        ...
        for ($row = 5; $row <= $highestRow; $row++)
        {
            ...
            {
              $insertSQL = sprintf("INSERT INTO ".$database_connect_prefix."cadre_logique (intitule,niveau,code,parent,projet, id_personnel) VALUES (%s, %s, %s, %s, %s, '$personnel')",
...
if ((isset($_POST["id_val"]) && !empty($_POST["id_val"]))) {
      ...
      $insertSQL = sprintf("DELETE from ".$database_connect_prefix."cadre_logique WHERE code in ($id)");
...
if ((isset($_GET["id_sup"]) && !empty($_GET["id_sup"]))) {
      $id = ($_GET["id_sup"]);
      $insertSQL = sprintf("DELETE from ".$database_connect_prefix."cadre_logique WHERE code=%s",
...
if ((isset($_POST["MM_form"])) && ($_POST["MM_form"] == "form1"))
{
  ...
  if ((isset($_POST["MM_update"]) && !empty($_POST["MM_update"]))) {
    $date=date("Y-m-d"); $personnel=$_SESSION['clp_id']; $c=$_POST['id'];
  	...
  	$insertSQL = sprintf("UPDATE ".$database_connect_prefix."cadre_logique SET intitule=%s, niveau=%s, code=%s, parent=%s, projet=%s, modifier_par='$personnel', modifier_le='$date' WHERE projet=%s and code='$c'",
                         GetSQLValueString($_POST['intitule'], "text"),
                         GetSQLValueString($_POST['niveau'], "int"),
  					     GetSQLValueString($_POST['code'], "text"),
                         GetSQLValueString($_POST['parent'], "text"),
...
if ((isset($_POST["MM_form"])) && ($_POST["MM_form"] == "form2"))
{
  ...
  $form= $_POST['form']; $noms= $form['lib']; $nombre=count($noms); $types= $form['indic'];
    ...
    $insertSQL = sprintf("INSERT INTO ".$database_connect_prefix."cadre_config (nombre, libelle, type, structure, projet) VALUES (%s, %s, %s, %s, %s)",
  ...
  $form= $_POST['form']; $noms= $form['lib']; $nombre=count($noms); $types= $form['indic'];
    ...
    $query_liste_actpa = sprintf("SELECT nombre FROM ".$database_connect_prefix."cadre_config WHERE projet=%s",
   // GetSQLValueString($_SESSION['clp_structure'], "text"),
    GetSQLValueString($_SESSION['clp_projet'], "text"));
	try{
    ...
    if($totalRows_liste_actpa>0 && $row_liste_actpa["nombre"]==$nombre+1){
      $insertSQL = sprintf("DELETE from ".$database_connect_prefix."cadre_logique WHERE niveau=%s and projet=%s",
  ...
  try{
  	...
  	$insertSQL = sprintf("UPDATE ".$database_connect_prefix."cadre_config SET nombre=%s, libelle=%s, type=%s, structure=%s WHERE projet=%s",
  					   GetSQLValueString($nombre, "int"),
  					   GetSQLValueString(implode(",",$noms), "text"),
                       GetSQLValueString(implode(",",$types), "text"),
                       GetSQLValueString("00", "text"),
...
<div class="widget-header"> <h4><i class="icon-reorder"></i> Niveaux de r&eacute;sultats </h4>
  <?php
$query_entete = "SELECT * FROM ".$database_connect_prefix."cadre_config WHERE projet='".$_SESSION["clp_projet"]."' LIMIT 1";
    	try{
...
if(isset($niveau) && isset($libelle[$niveau])){ $lib = $libelle[$niveau]; if($niveau<count($libelle)) {
  ...
  echo do_link("new_cadre","","Ajout $lib","<i class=\"icon-plus\" btn-warning> Ajouter $lib </i>","simple","./","pull-right p11","get_content('new_cadre_logique.php','niveau=".($niveau+1)."','modal-body_add',this.title);",1,"",$nfile);
  //echo do_link("","plan_analytique_projet.php","Retour","Retour","","./","pull-right p11","",0,"",$nfile);
  }
}
if(isset($_SESSION["clp_niveau"]) && $_SESSION["clp_niveau"]==0)
  echo do_link("","","G&eacute;rer les niveaux","<i class=\"icon-plus\"> Gestion des niveaux </i>","simple","./","pull-right p11","get_content('new_cadre_niveau.php','','modal-body_add',this.title);",1,"",$nfile);
    }
?>
</div>
...
$query_liste_activite_1 = "SELECT * FROM ".$database_connect_prefix."cadre_logique WHERE $where and projet='".$_SESSION["clp_projet"]."' ORDER BY niveau,code ASC";
    	try{
...
if($type[$niveau]==1)
{
  $query_projet = "SELECT code,intitule FROM ".$database_connect_prefix."activite_projet WHERE niveau=1 and projet='".$_SESSION["clp_projet"]."' ";
    	try{
  ...
  $query_entete = "SELECT libelle FROM ".$database_connect_prefix."niveau_config WHERE projet='".$_SESSION["clp_projet"]."' LIMIT 1";
  	try{
    $entete = $pdar_connexion->prepare($query_entete);
    $entete->execute();
    $row_entete = $entete ->fetch();
...
<?php if($type[$niveau]==1) { ?>
                    <td><?php echo "<strong>$libelle1[0]</strong>"; ?></td>
<?php } ?>
...
<?php } ?>
                    ...
                    <?php
if(isset($_SESSION["clp_niveau"]) && $_SESSION["clp_niveau"]==0){
                    echo do_link("","","Modifier ".$libelle[$niveau],"","edit","./","","get_content('new_cadre_logique.php','id=$code&niveau=".($niveau+1)."','modal-body_add',this.title);",1,"margin:0px 5px;",$nfile);
...
<?php if(isset($_SESSION['clp_niveau']) && ($_SESSION['clp_niveau']==0) && !check_user_auth('page_edit',$nfile)) { ?>
...
<?php } else {
if(isset($_SESSION["clp_niveau"]) && $_SESSION["clp_niveau"]==0)
  echo do_link("","","G&eacute;rer les niveaux","<i class=\"icon-plus\"> Gestion des niveaux </i>","","./","pull-right p11","get_content('new_cadre_niveau.php','','modal-body_add',this.title);",1,"",$nfile);
