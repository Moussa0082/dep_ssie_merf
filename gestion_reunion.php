...
if(isset($_GET["id_sup"]))
{
  ...
  $query_sup_act = "DELETE FROM ".$database_connect_prefix."reunions WHERE id_reunion='$id'";
...
{ //Atelier
  ...
  $insertSQL = sprintf("INSERT INTO ".$database_connect_prefix."reunions (type_reunion, objectif, lieu, participants, debut, recommandation, projet, id_personnel, date_enregistrement) VALUES (%s, %s, %s, %s, %s, %s, %s, '$personnel', '$date')",

					   GetSQLValueString($_POST['type_reunion'], "text"),
    ...
    $id = $_POST["MM_delete"];
    $insertSQL = sprintf("DELETE from ".$database_connect_prefix."reunions WHERE id_reunion=%s",
  ...
  $insertSQL = sprintf("UPDATE ".$database_connect_prefix."reunions SET type_reunion=%s, objectif=%s, lieu=%s, participants=%s, debut=%s, recommandation=%s, etat='Modifié', modifier_par='$personnel', modifier_le='$date' WHERE id_reunion='$id'",
...
//TDR
  ...
  $insertSQL = sprintf("UPDATE ".$database_connect_prefix."reunions SET recommandation=%s, ".(($Result1)?" rapport=".GetSQLValueString($link, "text").", ":"")." etat='Modifié', modifier_par='$personnel', modifier_le='$date' WHERE id_reunion='$id'",
...
<div class="widget-header"> <h4><i class="icon-reorder"></i><strong> R&eacute;unions et Rencontres </strong></h4>
<?php if(isset($_SESSION['clp_niveau']) && $_SESSION['clp_niveau']==1){ ?>
<?php
//echo do_link("","","Nouvel at&eacute;lier","<i class=\"icon-plus\"> Nouvel </i>","","./","pull-right p11","get_content('new_reunion.php','annee=$annee','modal-body_add',this.title);",1,"",$nfile);
?>
<?php } ?>
</div>
...
<div class="tabbable tabbable-custom" >
  ...
  <?php for($j=$_SESSION["annee_debut_projet"];$j<=$_SESSION["annee_fin_projet"];$j++){ ?>
  <div class="tab-pane <?php echo ($j==$annee)?"active":""; ?>" id="tab_feed_<?php echo $j; ?>" data-target="./gestion_reunion_content.php?annee=<?php echo $j; ?>"></div>
  <?php }  ?>
  </div>
</div>
