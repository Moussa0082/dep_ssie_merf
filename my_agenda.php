...
if ((isset($_POST["MM_form"])) && ($_POST["MM_form"] == "form1"))
{
  ...
  if(!isset($_POST['all_day']))
  {
    ...
    $insertSQL = sprintf("INSERT INTO ".$database_connect_prefix."agenda_perso (titre, description, all_day, debut, fin, couleur, lien, `type`, id_personnel) VALUES (%s, %s, %s, %s, %s, %s, %s, 'private', '$personnel')",
                        ...
                        //GetSQLValueString($_SESSION['clp_id'], "text") , expediteur , %s
                        );
  ...
  if(!isset($_POST['all_day']))
  {
    ...
    $insertSQL = sprintf("UPDATE ".$database_connect_prefix."agenda_perso SET titre=%s, description=%s, all_day=%s, debut=%s, fin=%s, couleur=%s, lien=%s, valider=%s, modifier_par='$personnel', modifier_le='$date' WHERE id_agenda=%s",
                        GetSQLValueString($_POST['titre'], "text"),
                        GetSQLValueString($_POST['description'], "text"),
  					    GetSQLValueString((!isset($_POST['all_day']))?0:1, "int"),
                        GetSQLValueString($debut, "date"),
...
$query_liste_event = "SELECT *, DATEDIFF(debut,fin) as date_diff, TIMEDIFF(debut,fin) as time_diff, DATEDIFF(now(),debut) as debut_date_diff, TIMEDIFF(now(),debut) as debut_time_diff, DATEDIFF(now(),fin) as fin_date_diff, TIMEDIFF(now(),fin) as fin_time_diff FROM ".$database_connect_prefix."agenda_perso WHERE (`type`='public' and expediteur='".$_SESSION['clp_id']."') OR (id_personnel='".$_SESSION['clp_id']."' and `type`='private')";
      	try{
...
$description = ((!empty($row_liste_event['expediteur']))?"<b>Auteur : </b>".$row_liste_event['expediteur']."<br />":"")."<b>Description : </b>".$row_liste_event['description']."<br /><b>D&eacute;but :</b> ".date_reg($row_liste_event['debut'],"/",1)."<br /><b>Fin : </b> ".(($row_liste_event['all_day']==1)?"toute la journ&eacute;e":date_reg($row_liste_event['fin'],"/",1));
...
$("#msg_00").attr("onclick","get_content('new_event.php','date="+dd+"','modal-body_add',this.title);");
$("#msg_00").click();
}
            },
eventDrop: function(event, dayDelta, minuteDelta, allDay, revertFunc) { $(".popover").popover('hide');
...
$("#msg_00").attr("onclick","get_content('new_event.php','id="+calEvent.id+"','modal-body_add',this.title);");
$("#msg_00").click();
},
...
<div class="widget box ">
 <div class="widget-header"> <h4><i class="icon-reorder"></i>Mon Agenda</h4>
<?php //if(isset($_SESSION['clp_niveau']) && $_SESSION['clp_niveau']==1){ ?>
<?php
  echo do_link("","","Ajout d'&eacute;v&egrave;nement","Ajout d'&eacute;v&egrave;nement","","./","pull-right p11","get_content('new_event.php','','modal-body_add',this.title);",1,"",$nfile);
?>
<?php //} ?>
</div>
<div class="widget-content">
<?php
echo do_link("msg_00","","Evenement","Evenement","","./","hidden","get_content('new_event.php','','modal-body_add',this.title);",1,"",$nfile)
?>
<div id="calendar"></div>
</div>
