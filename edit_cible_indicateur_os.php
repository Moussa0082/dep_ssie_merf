...
{   $id_ind=$_POST['id']; $prd=$_POST['prd'];
 ...
 // $id_region=$_POST['id_region'];
  ...
  //$idzone=$id_zone[$key];
     ...
     $insertSQL = sprintf("DELETE from cible_cmr_os WHERE indicateur_os=%s",
 ...
 //echo "je suis la";
  ...
  elseif(trim(strtolower($valind[$key]=="non"))) $valind[$key] = "1";
    ...
    $insertSQL = sprintf("INSERT INTO ".$database_connect_prefix."cible_cmr_os  (indicateur_os, annee, valeur_cible, id_personnel, date_enregistrement) VALUES (%s, %s, %s, '$personnel', '$date')",
  ...
  $("#acharger<?php echo $prd; ?>", window.parent.document).html(get_content('cmr_os_reload.php','id=<?php echo $prd; ?>','acharger<?php echo $prd; ?>','','',1));
...
if(isset($_GET["id_ind"])){ $id_ind=$_GET['id_ind']; // $annee=$_GET['annee'];
   ...
   /* mysql_select_db($database_pdar_connexion, $pdar_connexion);
	...
	$query_indicateur_courant = "SELECT * FROM indicateur_os_cmr, os, indicateur_os where id_indicateur_os=indicateur_prd and id_os=os and id_indicateur=$id_ind";
	...
	$totalRows_indicateur_courant  = mysql_num_rows($indicateur_courant);*/
			...
			$query_cible_indicateur = "SELECT id_cible_cr, annee, valeur_cible FROM   ".$database_connect_prefix."cible_cmr_os where indicateur_os='$id_ind'";
						try{
    ...
    $totalRows_cible_indicateur = $cible_indicateur->rowCount();
			...
			$query_cible_tindicateur = "SELECT sum(valeur_cible) as cible_total FROM   ".$database_connect_prefix."cible_cmr_os where indicateur_os='$id_ind' group by zone";
