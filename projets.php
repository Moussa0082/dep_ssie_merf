...
if ((isset($_POST["MM_form"])) && ($_POST["MM_form"] == "form1"))
{
  ...
  $insertSQL = sprintf("UPDATE ".$database_connect_prefix."projet SET code_projet=%s, sigle_projet=%s, annee_debut=%s, annee_fin=%s, ugl=%s, adresse=%s, contact=%s, intitule_projet=%s, actif=%s WHERE id_projet='$c'",
  			   //GetSQLValueString($_SESSION["clp_structure"], "text"),
               GetSQLValueString($_POST['code_projet'], "text"),
  			   GetSQLValueString($_POST['sigle_projet'], "text"),
               GetSQLValueString($_POST['annee_debut'], "int"),
...
$mySqlQuery = "SELECT * FROM ".$database_connect_prefix."projet  where FIND_IN_SET( code_projet, '".$pf_projet."' ) ORDER BY code_projet";
...
//Structure
$query_structure = "SELECT code_structure,sigle,nom_structure FROM ".$database_connect_prefix."structure ";
try{
...
$liste_structure_arrayV[$row_structure["code_structure"]]=$row_structure["nom_structure"];
$liste_structure_array[$row_structure["code_structure"]]=$row_structure["sigle"];
} }
//Projet/Structure
...
<!--<tr><td width="30%">Structure :</td><td><b><?php $a = explode("|",(implode('|',$liste_projet_array[$retData[$i]['code_projet']]))); if(count($a)>0){ $c = array(); foreach($a as $b) $c[]="<span title=\"".$liste_structure_arrayV[$b]."\">".$liste_structure_array[$b]."</span>";//do_link("","","Zone d'intervention de : ".$liste_structure_array[$b],"<span title=\"".$liste_structure_arrayV[$b]."\">".$liste_structure_array[$b]."</span>","","./","","get_content('new_projet_zone.php','id=$b&projet=$code','modal-body_add',this.title);",1,"margin:0px 5px;",$nfile);
echo implode('; &nbsp;',$c); } else echo "Aucun"; ?></b></td></tr>-->
<tr><td width="30%">Entit&eacute; de gestion :</td><td><b><?php $a = explode("|",$retData[$i]['ugl']); if(count($a)>0 && !empty($retData[$i]['ugl'])){ $c=array(); foreach($a as $b) if(isset($tableauUgl[$b])) $c[]="<span title=\"".$tableauUglV[$b]."\">".$tableauUgl[$b]."</span>";//do_link("","","Entit&eacute; de gestion de : ".$tableauUgl[$b],"<span title=\"".$tableauUglV[$b]."\">".$tableauUgl[$b]."</span>","","./","","get_content('new_projet_zone.php','id=$b&projet=$code','modal-body_add',this.title);",1,"margin:0px 5px;",$nfile);
