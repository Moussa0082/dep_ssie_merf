...
if ((isset($_GET["id_sup"]) && intval($_GET["id_sup"])>0)) {
  ...
  if($Result1 && (count($email_to)>0 || count($email_cc)>0))
  {
      ...
      $lien = $config->lien;
      $_GET["titre"] = "Suppression d'utilisateur - NIGELEC";
      //$partenaire = "";
...
//print_r($liste_projet_array); exit;
  ...
  if ((isset($_POST["MM_update"]) && intval($_POST["MM_update"])>0)) {
    ...
    if($Result1 && (count($email_to)>0 || count($email_cc)>0))
    {
        if(count($email_to)<=0 && count($email_cc)>0) { $email_to = $email_cc; unset($email_cc); }
        $lien = $config->lien;
        $_GET["titre"] = "Modification de compte d'utilisateur - NIGELEC";
        $partenaire = "";
 	    if(isset($partenaire_array[$_POST["partenaire"]])) $partenaire=$partenaire_array[$_POST["partenaire"]];
        ...
        $_GET["message"] = "<h3>Cher utilisateur/partenaire, <h3><h5>Dans le cadre de votre collaboration avec NIGELEC, le compte d'utilisateur auquel vous acc&eacute;dez au syst&egrave;me de Suivi-Evaluation du NIGELEC vient d'&ecirc;tre modifi&eacute; : </h5><div>Identifiant : <b>".$_POST['id_personnel']."</b></div><div>".(!empty($_POST['password'])?"Nouveau ":"")."Mot de passe".(empty($_POST['password'])?" (<small>inchang&eacute;</small>)":"")." : <b>".(!empty($_POST['password'])?$_POST['password']:"")."</b></div><div>Pr&eacute;nom(s) et Nom : <b>".$_POST['prenom']." ".$_POST['nom']."</b></div><div>Date de cr&eacute;ation : <b>".(!empty($date)?date_reg($date,"/"):"-")."</b></div><div>Fonction : <b>".(!empty($_POST["fonction"])?((isset($fonction_array[$_POST["fonction"]]))?$fonction_array[$_POST["fonction"]]:$_POST["fonction"]):"-")."</b></div><div>Partenaires : <b>".$partenaire."</b></div>";
        include("./phpmailer/mail_notification.php");
        if (!isset($msg_sent) || $msg_sent!=1) $insertGoTo .= "&send=no"; else $insertGoTo .= "&send=ok";
        //header(sprintf("Location: %s", $insertGoTo)); exit;
    }
...
<th class="sorting" role="" tabindex="0" aria-controls="" aria-label="" width="120"><?php if(isset($_SESSION['clp_niveau']) && $_SESSION['clp_niveau']==1) { ?>Actions<?php } ?></th>
<?php //} ?>
</tr>
</thead>
...
<td class=" " align="center"><?php if(isset($_SESSION['clp_id']) && $_SESSION["clp_id"]=="admin") { ?>
<?php
 if($row_personnel['id_personnel']!="admin") echo do_link("new_user_auth_$id","","Droits d'utilisateur ".$row_personnel['nom'],"","access","./","","get_content('new_user_auth.php','id=$id','modal-body_add',this.title);",1,"margin:0px 10px 0 0;",$nfile);
...
echo do_link("","","Modifier utilisateur ".$row_personnel['id_personnel'],"","edit","./","","get_content('new_user.php','id=$id','modal-body_add',this.title);",1,"margin:0px 5px;",$nfile);
 ...
 if($row_personnel['id_personnel']!="admin") echo do_link("",$_SERVER['PHP_SELF']."?id_sup=".$id,"Supprimer","","del","./","","return confirm('Voulez-vous vraiment supprimer cet utilisateur ".$row_personnel['id_personnel']."');",0,"margin:0px 5px;",$nfile);
