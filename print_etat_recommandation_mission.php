...
$pdf->SetAuthor('Ruche');
$pdf->SetTitle('Etat des recommandations missions');
$pdf->SetSubject('Mission de R&eacute;commandation');
...
//observations
/*mysql_select_db($database_pdar_connexion, $pdar_connexion);
  $query_suivi_plan_ms = "SELECT observation, code_rec  FROM ".$database_connect_prefix."mission_plan where code_ms='$id' and valider=1 order by code_rec";
  ...
  $observation_tab = array();
  if($totalRows_suivi_plan_ms>0){  do{
    $observation_tab[$row_suivi_plan_ms["code_rec"]]=$row_suivi_plan_ms["observation"];
...
<table width="<?php echo (!isset($_GET["down"]))?'':'100%'; ?>" border="<?php echo (!isset($_GET["down"]))?0:1; ?>" cellspacing="0" class="table table-striped table-bordered table-hover table-responsive">
            ...
            <?php foreach($row_liste_rec as $row_liste_rec){ $id=$row_liste_rec["id_recommandation"]; ?>
...
$tauxp=0;
 ...
 elseif($tauxp>=100) $color = "green";
    }elseif(date("Y-m-d")>$row_liste_rec['date_buttoir'] && $row_liste_rec['type']!="Continu") $color = "red";
	 else $color = "#666666";
	
	 //else{  $color = "#FFD700"; }  ?>
<div> 
<div style="width: 100%; background-color: <?php echo $color; ?>; color:#FFFFFF;">
<?php if(isset($prop_tab[$id])) echo $prop_tab[$id]." %"; elseif(date("Y-m-d")>$row_liste_rec['date_buttoir'] && $row_liste_rec['type']!="Continu") echo "Non entam&eacute;e"; else echo "Non &eacute;chu"; ?></div></div></td>
             <td width="300"><div align="left" class="Style4"><?php echo $row_liste_rec['observation']; ?></div></td>
            </tr>
            <?php }  ?>
            <?php } else { ?>
            <tr>
              <td colspan="7"><div align="center"><span class="Style4"><em><strong>Aucune recommandation enregistr&eacute;e! </strong></em></span><span class="Style4"></span><span class="Style4"></span><span class="Style4"></span><span class="Style4"></span></div></td>
            </tr>
            <?php }  ?>
        </table>
        <hr id="sp_hr" />
