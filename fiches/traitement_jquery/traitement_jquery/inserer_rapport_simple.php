...
$t=time();
	extract($_POST);
	if(isset($_GET["action"]) AND $_GET["action"]=="modif"){PC_Enregistrer_Code("DELETE FROM t_rapport WHERE Code_Rapport=".base64_decode($Code_Rapport));}
...
foreach (FC_Rechercher_Code('SELECT * FROM t_feuille_ligne WHERE (Code_Feuille='.$select_feuille." AND Nom_Ligne='".$input_regrouper_par."')") as $row7)
	...
	PC_Enregistrer_Code("INSERT INTO t_rapport(Nom_Rapport, Code_Feuille, Group_By, Valeur, Operation,  Nom_View, Structure_View, Type_Rapport, Feuille_Jointure, Attribut_Jointure_FP, Attribut_Jointure_FS) VALUES ('".$nom_rapport."', $select_feuille, '".$input_regrouper_par."', '".$input_valeur."', '".$operation."', '".str_replace("t", "vw_r", $row4["Table_Feuille"])."_".$t."', '', 'SIMPLE', $FEUILLE_JOINTURE, '$attribut_jointure_fp', '$attribut_jointure_fs')");
$IND=0;
