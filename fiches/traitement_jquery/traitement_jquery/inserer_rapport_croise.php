...
foreach (FC_Rechercher_Code('SELECT * FROM t_feuille_ligne WHERE (Code_Feuille='.$select_feuille." AND Nom_Ligne='".$colonne_y."')") as $row13)
	...
	PC_Enregistrer_Code("INSERT INTO t_rapport(Nom_Rapport, Code_Feuille, Colonne_X ,Colonne_Y, Valeur, Operation,  Nom_View, Structure_View, Type_Rapport, Feuille_Jointure, Attribut_Jointure_FP, Attribut_Jointure_FS) VALUES ('".$nom_rapport."', $select_feuille, '".$colonne_x."', '".$colonne_y."', '".$input_valeur."', '".$operation."', '".str_replace("t", "vw_r", $row4["Table_Feuille"])."_".$t."', '', 'CROISE', $FEUILLE_JOINTURE, '$attribut_jointure_fp', '$attribut_jointure_fs')");
$IND=0;
