...
if(isset($_GET['code_feuille']) && $_GET['code_classeur']){

    $codeFeuille = $_GET['code_feuille'];
    $codeClasseur = $_GET['code_classeur'];

 $action = PC_Enregistrer_Code("UPDATE t_feuille SET Code_Classeur='".addslashes($codeClasseur)."'  WHERE Code_Feuille=".$codeFeuille);
...
<div id="add_form_perso_modal2" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true"  style="width: 100%">
...
<!-- dep feuille -->
            ...
            <style>
                ...
                .classeurliste_:hover,
                .classeurliste_:focus,
                .classeurliste_:active{
       ...
       <div id="classeurList" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" >
                <div class="modal-dialog modal-sm"  role="document" >
                    <div class="modal-content" id="add_form_perso_modal_body2">
                        <div class="modal-header">
                            <button id="fermer" type="button" class="close" data-dismiss="modal" aria-hidden="true"><img src="./images/close.png" alt="Fermer"></button>
                            <center><h4 class="modal-title" id="add_form_perso_modal_titre2">Déplacer vers Classeur</h4></center>
                        </div>
                        <div  class="row">
                            <div class="col-lg-12" id="list_classeur_for_move">
                                <?php
                               ...
                               <?php   if ($row3['Code_Classeur'] != $Code_Classeur ) :  ?>
                                    <div class="classeurliste_" onclick="Move_to_sheet('<?=  $row3['Code_Classeur']  ?>')"><?php  echo $row3['Libelle_Classeur'] ?></div>
                               <?php  endif; ?>
...
<script type="text/javascript">
    ...
    var codeFeuilleTomove = ""


    function move_sheet(Code){
        $('#classeurList').modal()
        codeFeuilleTomove = Code
    }


    function Move_to_sheet(code_classeur){
        $('#list_classeur_for_move').html("<div class='text-center'><img src='images/loading-bars.svg'  style='text-align: center ;padding : 20px' /> <br> <small>Déplacement en cours </small> </div>")
         $.ajax({
             url : "",
             type: "get",
             data : {
                 code_feuille : codeFeuilleTomove,
                 code_classeur : code_classeur
             },
             success:function (msg) {
                if (msg.trim() == "true"){
                    $('#list_classeur_for_move').html("<div class='text-center text-success'>Déplacement éffectué</div>")
                    setTimeout(()=>{
                        window.location.reload(true)
                    },2000)
                }else{
                    $('#list_classeur_for_move').html("<div class='text-center text-danger'>Erreur lors du déplacement</div>")
                }
             }
         })
    }
 ...
 {   $.ajax({url:"traitement_jquery/formulaire_modification_feuille.php?feuille="+Code+"&classeur=<?php echo $_GET['c']; ?>", method:"POST", data:$('#form_feuille_insertion').serialize(), success:function (data) {if(data!='')
  ...
  document.getElementById('form_feuille_modification').addEventListener('submit',function(e){
    ...
    $.ajax({url:"traitement_jquery/modification_feuille.php", method:"POST", processData:false, contentType:false, data:formData, success:function (data) {
        if(data=='')
         {var pathArray = window.location.pathname.split('/');var Url = pathArray.length>2?pathArray[pathArray.length-1]:pathArray[1];/*var Url=window.location.pathname.substring(1);*/ window.location.href=(Url+"?c="+Code_Classeur);}
         else{}
                                                }
    });
    });
...
if(Nb<1)
 ...
 {   $.ajax({url:"traitement_jquery/formulaire_afficher_position.php?Id="+Id+"&feuille="+Code, method:"POST", data:$('#form_feuille_insertion').serialize(), success:function (data) {if(data!='')
        {$("#Form_Contenu_Perso2").html(data);
        $("#add_form_perso_modal_titre2").html("Metadonnées");
         document.getElementById("boutton_nouvelle_feuille_perso").style.display="none";
         document.getElementById("div_conseil").style.display="none";
         $("#add_form_perso_modal2").modal('show');
    }else {}}});
   ...
   function Afficher_Signature(Sign)
 {   $("#Form_Contenu_Perso2").html(atob(Sign));
     $("#add_form_perso_modal_titre2").html("Signature");
...
<!--                         <li>-->
<!--                            <a onClick="--><?php //echo 'move_sheet(\''.$row5['Code_Feuille'].'\')'; ?><!--"><span class="glyphicon glyphicon-refresh text-success"></span>-->
<!--                                <span class="text-success">Déplacer la feuille</span>-->
<!--                            </a>-->
<!--                        </li>-->
...
function Afficher_Formulaire_Insertion_Donnees(Feuille, Id)
{
  $.ajax({url:"traitement_jquery/generation_formulaire_modification.php?Code="+Feuille+"&Id="+Id, method:"POST", data:$('#form_feuille_modification').serialize(), success:function (data) {if(data!='')
        { $("#add_form_perso_modal_body").html(data);
        //$("selectpicker").addClass("form-control selectpicker bts_select ");
        $(".selectpicker").selectpicker("refresh");
          $("#add_form_perso_modal").modal('show');
...
echo '<th style="text-align: center; vertical-align : middle"><input type="checkbox" name="" id="checkbox_valider_th'.$row5['Code_Feuille'].'" class="" style="display: none;"></th>';
    echo '<th  style="vertical-align : middle">#</th>';
    echo '<th  style="vertical-align : middle">#</th>';
    echo '<th style="border-right:1px solid silver; vertical-align : middle">#</th>';
    if($row5['Code_Feuille'] == $Code_Feuille_Carte){echo '<th style="border-right:1px solid silver; vertical-align : middle">Carte</th>';}
 $Nb_Col=0;   $compte=0;
 $Tab_Col_Entete = null;
 $Tab_Col_Entete_Type = null;
    ...
    {echo '<th style="vertical-align : middle">'.$row6['Libelle_Ligne'].'</th>';$Nb_Col++; 
  $Tab_Col_Entete[]=$row6['Nom_Collone']; $Tab_Col_Entete_Type[]=$row6['Type_Ligne'];}
    ...
    try{
      ...
      if($row5["Source_Donnees"] == 'Oui')
        {$Res=FC_Rechercher_Code('SELECT * FROM '.str_replace("t", "v", $row5['Table_Feuille']).' ORDER BY Stat ASC, Id DESC');}
      else {$Res=FC_Rechercher_Code('SELECT * FROM '.str_replace("t", "v", $row5['Table_Feuille']).$Filtre_Admin.' ORDER BY Stat ASC, Id DESC');}
      
    
    if($Res!=null){
    foreach ($Res as $row6)
    {
    echo '<tr style="margin:0px!important; padding:0px!important">'; $compte++;
    ...
    echo '<td style="background-color:#F1F3F6; text-align: center; width: 20px"><span class="glyphicon glyphicon-map-marker text-primary" style="cursor: pointer; width:20px" onclick="Afficher_Position(\''.$row5['Code_Feuille'].'\','.$row6['Id'].')"></span></td>
      ...
      for($j=0; $j<count($Tab_Col_Entete); $j++){
        ...
        switch ($Tab_Col_Entete_Type[$j]){
            case 'FEUILLE' :
            echo '<td><span  style="color:blue; font-weight: bold">'.$row6[$Tab_Col_Entete[$j]].'</span></td>';
...
{echo base64_encode($row10xx["Code_Classeur"]).'&f='.$row10xx["Code_Feuille"];}
            echo '&search='.urlencode($row6[$row6['Nom_Collone']]).'"><span  style="color:blue; font-weight: bold">'.$row6[$row6['Nom_Collone']].'</span></a></td>';*/
              ...
              break;
            case 'TEXT' : case 'DATE' : case 'CHOIX' : case 'QRCODE' : case 'CHOIX MULTIPLES' :
            echo '<td>'.$row6[$Tab_Col_Entete[$j]].'</td>';
                break;
            case 'INT' :
            echo '<td>'.number_format($row6[$Tab_Col_Entete[$j]],0, '',' ').'</td>';
                break;
            case 'DOUBLE' :
            echo '<td>'.number_format($row6[$Tab_Col_Entete[$j]],2, '.',' ').'</td>';
                break;
            ...
            case 'COULEUR' :
            echo '<td><div style="border-radius: 50% ;height: 20px ; background-color:'.$row6[$Tab_Col_Entete[$j]].'"></div></td>';
                break;

            case 'FICHIER' :
            echo '<td align="center">';
            if(!empty($row6[$Tab_Col_Entete[$j]]))
                {echo '<a href="pieces/'.$row6[$Tab_Col_Entete[$j]].'" download="'.$row6[$Tab_Col_Entete[$j]].'" style="font-size: 14px"><span class="glyphicon glyphicon-paperclip"></span></a>';}
            echo '</td>';
                break;
            ...
            echo '<td align="center">';
            if(!empty($row6[$Tab_Col_Entete[$j]]))
                {echo '<a href="#"  onclick="Afficher_Signature(\''.base64_encode(str_replace(array('<?xml version="1.0" encoding="UTF-8" standalone="no"?>', "\r", "\n"), '',$row6[$Tab_Col_Entete[$j]])).'\')" style="font-size: 14px">Voir la signature</a>';}
            echo '</td>';
                break;
...
{    var date=new Date();
    ...
    if(Id=="CHOIX")
 {var DIV_CHOIX='<div class="col-sm-2 col-md-2 mb-2"></div><div class="col-sm-6 col-md-6 mb-6"><input type="text" class="form-control" required placeholder="Separer les valeurs par point-virgule \';\' Exemple: HOMME ; FEMME"  onkeyup="document.getElementById(\'Valeur_'+Code+'\').value=this.value"></div>';
    document.getElementById('Under_Div_'+Code).innerHTML=DIV_CHOIX;}
else     if(Id=="CHOIX MULTIPLES")
 {var DIV_CHOIX='<div class="col-sm-2 col-md-2 mb-2"></div><div class="col-sm-6 col-md-6 mb-6"><input type="text" class="form-control" required placeholder="Separer les valeurs par point-virgule \';\' Exemple: HOMME ; FEMME"  onkeyup="document.getElementById(\'Valeur_'+Code+'\').value=this.value"></div>';
    document.getElementById('Under_Div_'+Code).innerHTML=DIV_CHOIX;}
 else if(Id=="FEUILLE")
...
{$("#panel-body").append('<div class="div_ligne_feuille" id="div_ligne_feuille_'+(document.getElementsByClassName("div_ligne_feuille").length+1)+'"><div class="row" ><br><div class="col-sm-2 col-md-2 mb-2"><textarea type="text"  style="height: 50px !important;" class="form-control" placeholder="Nom Ligne" required  name="nom_Ligne[]" id="nom_Ligne[]"></textarea></div><div class="col-sm-2 col-md-1.5 mb-2"><textarea type="text" required class="form-control"  style="height: 50px !important;" placeholder="Libelle" name="libelle_Ligne[]" id="libelle_Ligne[]"></textarea></div><div class="col-sm-2 col-md-2 mb-2"><select class="form-control" required placeholder="" onchange="Charger_Div(this.value, '+(document.getElementsByClassName("div_ligne_feuille").length+1)+')" name="type_Ligne[]" id="type_Ligne[]"><option value="TEXT">TEXT</option><option value="INT">INT</option><option value="DOUBLE">DOUBLE</option><option value="DATE">DATE</option><option value="CHOIX">CHOIX</option><option value="COULEUR">COULEUR</option><option value="FICHIER">FICHIER</option><option value="FEUILLE">FEUILLE</option><option value="RAPPORT">RAPPORT</option><option value="SOMME">SOMME</option><option value="DIFFERENCE">DIFFERENCE</option><option value="PRODUIT">PRODUIT</option><option value="MOYENNE">MOYENNE</option><option value="COMPTER">COMPTER</option><option value="QRCODE">QRCODE</option><option value="CHOIX MULTIPLES">CHOIX MULTIPLES</option><option value="SIGNATURE">SIGNATURE</option></select></div><div class="col-sm-2 col-md-2 mb-2"><select class="form-control" placeholder="" required name="requis[]" id="requis[]"><option value="Oui">Oui</option><option value="Non">Non</option></select><input type="hidden" name="valeur[]" id="Valeur_'+(document.getElementsByClassName("div_ligne_feuille").length+1)+'"></div><div class="col-sm-2 col-md-2 mb-2"><select class="form-control" placeholder="" required name="afficher[]" id="afficher[]"><option value="Oui">Oui</option><option value="Non">Non</option></select></div><div class="col-sm-1 col-md-1 mb-1"></div><div class="col-sm-1 col-md-1 mb-1"><div class="btn btn-xs btn-default" onclick="Supprimer_Item(\''+(document.getElementsByClassName("div_ligne_feuille").length+1)+'\')" title="Supprimer" ><span class="nav-label glyphicon glyphicon-remove text-danger" ></span></div></div><br></div><div class="row" id="Under_Div_'+(document.getElementsByClassName("div_ligne_feuille").length+1)+'"></div></div>');}
