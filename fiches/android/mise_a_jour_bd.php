...
if($ACCESS=="O"){
$Code= "[";
foreach (FC_Rechercher_Code("SELECT * FROM 
   (personnel INNER JOIN t_feuille_partenaire ON (personnel.partenaire = t_feuille_partenaire.code)) 
   			  INNER JOIN t_feuille ON (t_feuille_partenaire.Code_Feuille=t_feuille.Code_Feuille)
			  INNER JOIN t_classeur ON (t_classeur.Code_Classeur=t_feuille.Code_Classeur) WHERE (personnel.id_personnel ='".$_POST['Login']."')") as $row1)
{
	/*$Code.="{\"Code\" : \"INSERT INTO t_convention(Code_Convention, Libelle_Convention) VALUES (".$row1['id_convention'].", 'C - ".$row1['id_convention']." / ".str_replace("'", "''", $row1['sigle_partenaire'])."');\"}, ";*/

	$Code.="{\"Code\" : \"INSERT INTO t_classeur(Code_Classeur, Code_Convention, Libelle_Classeur, Note_Classeur, Couleur_Classeur, Date_Insertion) VALUES (".$row1['Code_Classeur'].", 0, '".str_replace("'", "''", $row1['Libelle_Classeur'])."', '".str_replace("'", "''",$row1['Note_Classeur'])."', '".$row1['Couleur_Classeur']."', '".$row1['Date_Insertion']."');\"}, ";


$Structure_Table="";
$ind=0;
 $Structure_Table.="CREATE TABLE ".$row1['Table_Feuille']." (Id INTEGER PRIMARY KEY AUTOINCREMENT, Stat INTEGER DEFAULT 0,  SENT INTEGER DEFAULT 0, LOGIN VARCHAR(255) DEFAULT NULL, LG VARCHAR(255) DEFAULT NULL, LT VARCHAR(255) DEFAULT NULL ,";
	foreach (FC_Rechercher_Code('DESCRIBE '.$row1['Table_Feuille']) as $row4)
	{if($ind>=6){
...
$Structure_Table.=");";	
	$Code.="{\"Code\" : \"INSERT INTO t_feuille(Code_Feuille, Code_Classeur, Nom_Feuille, Libelle_Feuille, Nb_Ligne_Impr, Icone, Note, Table_Feuille, Structure_Table, Structure_View, Source_Donnees, Code_Convention) VALUES (".$row1['Code_Feuille'].", ".$row1['Code_Classeur'].", '".str_replace("'", "''",$row1['Nom_Feuille'])."', '".str_replace("'", "''",$row1['Libelle_Feuille'])."', ".$row1['Nb_Ligne_Impr'].", '".$row1['Icone']."', '".str_replace("'", "''",$row1['Note'])."', '".$row1['Table_Feuille']."', '".$Structure_Table."', '".$row1['Structure_View']."' , '".$row1['Source_Donnees']."', 0);\"}, ";

	$Code.="{\"Code\" : \"DROP TABLE IF EXISTS ".$row1["Table_Feuille"]." ;\"}, ";
	$Code.="{\"Code\" : \"".$Structure_Table."\"}, ";

	

foreach (FC_Rechercher_Code('SELECT * FROM t_feuille_ligne WHERE (Mobile=\'Oui\' AND Code_Feuille='.$row1["Code_Feuille"].')') as $row3)
{
...
foreach (FC_Rechercher_Code("SELECT * FROM t_feuille_etrangere WHERE Nom_Table='".$row1["Table_Feuille"]."'") as $row5)
{
...
foreach (FC_Rechercher_Code('SELECT * FROM t_feuille WHERE (Source_Donnees=\'Oui\' AND Code_Feuille='.$row1["Code_Feuille"].')') as $row6)
{
