...
$query_liste_projet = $db ->prepare('SELECT P.id_projet, P.code_projet, P.sigle_projet, P.intitule_projet, (SELECT CONCAT(sigle_partenaire,"|",nom_partenaire) FROM t_partenaires WHERE FIND_IN_SET(:type_partenaire,type_partenaire) and id_partenaire=P.agence_lead LIMIT 1) as agence_lead FROM t_projets P, t_projet_users D WHERE P.programme=:programme and P.id_projet=D.projet_up and D.structure_up=:structure and FIND_IN_SET(:user,D.personnel_up) ORDER BY P.code_projet asc'
...
$query_liste_projet->execute(array(':type_partenaire' => 1,':programme' => isset($_SESSION['programme'])?$_SESSION['programme']:0,':structure' => $_SESSION["structure"],':user' => $_SESSION["id"]));
