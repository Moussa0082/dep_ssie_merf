...
<ul class="nav nav-tabs" style="border-bottom:1px solid #F1F3F6; margin:0px!important; padding: 0px!important">
    <?php
$ind=0;
    foreach (FC_Rechercher_Code('SELECT * FROM t_feuille WHERE Code_Classeur=\''.$Code_Classeur.'\'') as $row5)
    {$ind++;$Test=0;
    if(isset($_GET['f']) AND !empty($_GET['f']))
...
$.ajax({url:"traitement_jquery/traitement_auto_formulaire.php", method:"POST", processData:false, contentType:false, data:formData, success:function (data) {if(data=='')
        {
var pathArray = window.location.pathname.split('/');var Url = pathArray.length>2?pathArray[pathArray.length-1]:pathArray[1];/*var Url=window.location.pathname.substring(1);*/ window.location.href=(Url+"?c="+Code_Classeur+"&f="+feuille_active);
        }else {}}});}
...
function Envoyer_Form_Auto_Modif(){
    ...
    var file = $file[0].files[0];
    formData.append("File",$file[0].files);
$.ajax({url:"traitement_jquery/traitement_auto_formulaire_modification.php", method:"POST", processData:false, contentType:false, data:formData, success:function (data) {if(data=='')
        {
var pathArray = window.location.pathname.split('/');var Url = pathArray.length>2?pathArray[pathArray.length-1]:pathArray[1];/*var Url=window.location.pathname.substring(1);*/ window.location.href=(Url+"?c="+Code_Classeur+"&f="+feuille_active);
        }else {}}});}
    ...
    <div class="col-lg-12">
        <div class="hpanel">
            <div class="panel-heading">
               <div align="right" style=""><a  class="btn '.$Boutton_Style.'" id="Btn_Valider_Donnees_'.$row5['Code_Feuille'].'" style="display: none">Valider<span class=""></span></a></div>
            ...
            <div class="panel-body" style="padding:0px!important; margin:0px!important">
                ...
                <table cellpadding="1" cellspacing="1" class="table" style="margin:0px!important; padding:0px!important">
                    <tr style=" background-color:#F1F3F6; text-align: center">';
echo '<th style="border-right:1px solid silver; background-color:#F1F3F6; text-align: center; width: 20px!important"><input type="checkbox" name="" id="checkbox_valider_th'.$row5['Code_Feuille'].'" class="" style="display: none"></th>';
 $Nb_Col=0;   $compte=0;
    ...
    echo '<th style="border-right:1px solid silver; text-align: center; width: 60px!important" colspan="3">Actions</th>';
    echo '</tr>';
    $indd=0; $indu=0; $i=0;
    try{
      $Res=null;
      $Res=FC_Rechercher_Code('SELECT * FROM t_feuille_ligne INNER JOIN ('.str_replace("t", "v", $row5['Table_Feuille']).' INNER JOIN t_feuille ON (t_feuille.Table_Feuille=\''.$row5['Table_Feuille'].'\')) ON (t_feuille_ligne.Code_Feuille=t_feuille.Code_Feuille) WHERE (Afficher=\'Oui\') ORDER BY Id DESC, Rang ASC');
    if($Res!=null){
    foreach ($Res as $row6)
    {var_dump($row6);
    ...
    echo '<td style="border:none; border-right:1px solid silver;">';
...
{echo base64_encode($row10xx["Code_Classeur"]).'&f='.$row10xx["Code_Feuille"];}
            ...
            echo'"><span  style="color:blue; font-weight: bold">'.$row6[$row6['Nom_Collone']].'</span></a></td>';

              break;
            case 'TEXT' : case 'DATE' : case 'CHOIX' :
    ...
    if($indu%$Nb_Col==0){
echo '
<td style="background-color:#F1F3F6; text-align: center; width: 20px"><span class="glyphicon glyphicon-map-marker text-primary" style="cursor: pointer" onclick="Afficher_Position(\''.$row5['Code_Feuille'].'\','.$row6['Id'].')"></span></td>
<td style="background-color:#F1F3F6; text-align: center; width: 20px"><span class="glyphicon glyphicon-pencil text-info" style="cursor: pointer" onclick="Afficher_Formulaire_Insertion_Donnees(\''.$row5['Code_Feuille'].'\','.$row6['Id'].')"></span></td>
<td style="border-right:1px solid silver; background-color:#F1F3F6; text-align: center; width: 20px"><span class="glyphicon glyphicon-remove text-danger" style="cursor: pointer" onclick="if(confirm(\'Voulez-vous supprimer cette ligne?\')){window.location.href=\'traitement_jquery/feuille_donnees_supprimer.php?c='.$_GET['c'].'&f='.$row5['Code_Feuille'].'&d='.$row6['Id'].'&tab='.$row5['Table_Feuille'].'\'}"></span></td>';
        echo '</tr>'; $indu=0; }
    $i++;
    */}}
}
...
{Btn_Valider_Donnees_'.$row5['Code_Feuille'].'.style.display="table";
...
{echo '<option value="'.$row8['Code_Feuille'].'">'.$row8['Nom_Feuille'].'</option>';}
echo '\'; ';
?>
