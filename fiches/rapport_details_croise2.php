...
foreach (FC_Rechercher_Code("SELECT * FROM t_feuille_ligne WHERE(Code_Feuille=".$row5["Code_Feuille"]." AND Nom_Ligne='".$row5["Colonne_Y"]."')") as $row6) 
{echo "<th><sub>".$row6["Nom_Ligne"]."</sub> | "; $COLONNE_Y=$row6["Nom_Collone"];}

foreach (FC_Rechercher_Code("SELECT * FROM t_feuille_ligne WHERE(Code_Feuille=".$row5["Code_Feuille"]." AND Nom_Ligne='".$row5["Colonne_X"]."')") as $row7) 
{echo "<sup>".$row7["Nom_Ligne"]."</sup></th>"; $COLONNE_X=$row7["Nom_Collone"];}
...
$Res1 = FC_Rechercher_Code("SELECT DISTINCT($COLONNE_X) FROM ".$row5['Nom_View']." LIMIT 12");
if($Res1!=null)
  {foreach ($Res1 as $row10) 
{ echo "<th>".$row10[0]."</th>"; $COLONNE_X_TAB[]=$row10[0];}}

$Res2 = FC_Rechercher_Code("SELECT DISTINCT($COLONNE_Y) FROM ".$row5['Nom_View']." LIMIT 12");
if($Res2!=null)
...
if($Res!=null){
if($Res->rowCount()!=0){foreach ($Res as $row8) {echo "<td>".number_format($row8[0],0, '',' ')."</td>";}}
else {echo "<td>-</td>";}
}
