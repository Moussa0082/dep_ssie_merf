...
if ((isset($_GET["id_sup_ind"]) && intval($_GET["id_sup_ind"])>0)) {
  ...
  $insertSQL = sprintf("DELETE FROM liste_indicateur_sygri WHERE id_indicateur_sygri_fida=%s",
...
{ //personnel
  ...
  $insertSQL = sprintf("INSERT INTO  liste_indicateur_sygri (intitule_indicateur_sygri_fida, niveau_sygri, referentiel, groupe_indicateur, ordre, id_personnel, date_enregistrement) VALUES (%s, %s, %s, %s, %s, '$personnel', '$date')",

					   GetSQLValueString($_POST['intitule_indicateur_sygri_fida'], "text"),
					   GetSQLValueString($_POST['niveau_sygri'], "int"),
  ...
  if ((isset($_POST["MM_update"]) && intval($_POST["MM_update"])>0)) {
    ...
    $date=date("Y-m-d"); $personnel=$_SESSION['clp_id'];
$insertSQL = sprintf("UPDATE liste_indicateur_sygri SET intitule_indicateur_sygri_fida=%s, niveau_sygri=%s, referentiel=%s, groupe_indicateur=%s, ordre=%s,modifier_par='$personnel', modifier_le='$date' WHERE id_indicateur_sygri_fida=$id",
...
mysql_select_db($database_pdar_connexion, $pdar_connexion);
$query_ind = "SELECT * FROM groupe_indicateur, liste_indicateur_sygri where id_groupe=groupe_indicateur order by id_indicateur_sygri_fida";
...
<div class="widget-header"> <h4><i class="icon-reorder"></i> Liste des indicateurs SYGRI  </h4>
  <?php if (isset ($_SESSION["clp_id"]) && ($_SESSION["clp_id"] == "admin")) {?>
<a onclick="get_content('new_sygri_fida.php','','modal-body_add',this.title);" data-toggle="modal" href="#myModal_add" data-backdrop="static" data-keyboard="false" title="Ajout d'indicateur SYGRI" class="btn btn-sm btn-warning pull-right p11" dir=""><i class="icon-plus"> Nouvel indicateur </i></a>
...
<thead>
<tr role="row">
<th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" aria-label="Trier">Groupe</th>
...
<th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" aria-label="Trier">Niveau</th>
...
<?php $i=0; do { $id=$row_ind['id_indicateur_sygri_fida']; ?>
<tr class="<?php echo ($i%2==0)?"odd":"even"; ?>">
<td class=" "><?php echo substr($row_ind['nom_groupe'],0,20)."..."; ?></td>
<td class=" "><?php echo $row_ind['ordre']; ?></td>
<td class=" " <?php if(!isset($unite_ind_ref_array[$row_ind["referentiel"]]) || $unite_ind_ref_array[$row_ind["referentiel"]]=="") {?>style="color:#FF0000"<?php } ?>><?php echo $row_ind['intitule_indicateur_sygri_fida']; ?></td>
<td class=" "><?php if(isset($unite_ind_ref_array[$row_ind["referentiel"]])) echo " (".$unite_ind_ref_array[$row_ind["referentiel"]].")"; ?></td>
<td class=" "><?php if(isset($row_ind['niveau_sygri']) && $row_ind['niveau_sygri']==1) echo "Premier"; elseif(isset($row_ind['niveau_sygri']) && $row_ind['niveau_sygri']==2) echo "Deuxième"; elseif(isset($row_ind['niveau_sygri']) && $row_ind['niveau_sygri']==3) echo "Troisième";   ?></td>
<td class=" " align="center"><a onclick="get_content('new_sygri_fida.php','id=<?php echo $id; ?>','modal-body_add',this.title);" data-toggle="modal" href="#myModal_add" title="Modification d'indicateur" class="thickbox Add"  dir=""><img src="images/edit.png" width='20' height='20' alt='Modifier' /></a></td>
