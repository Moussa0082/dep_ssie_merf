...
header("content-disposition: attachment;filename=tableau_PPM_par_categorie.xls"); }
...
header("content-disposition: attachment;filename=tableau_PPM_par_categorie.doc"); }
...
if (isset($_SERVER['QUERY_STRING'])) {
 ...
 $query_liste_marche = "SELECT *   FROM ".$database_connect_prefix."plan_marche where periode like '%$version%' and categorie='$modele' and projet='".$_SESSION["clp_projet"]."' order by  categorie, code_marche asc";
    ...
    $row_liste_marche1 = $liste_marche ->fetchAll();
...
/*$modele_array = array();
    if($totalRows_liste_modele>0){
	 foreach($row_liste_modele as $row_liste_modele){ $modele_array[$row_liste_modele["code"]]=$row_liste_modele['categorie']." - ".$row_liste_modele['methode_concerne']." - ".$row_liste_modele['examen']; }}*/
...
<div class="well well-sm"><strong>Tableau du plan de passation des march&eacute;s, en date du <?php echo date("d/m/Y"); ?></strong></div>

<h3><?php if(isset($row_liste_modele['nom_categorie'])) echo $row_liste_modele['nom_categorie']; ?></h3>
...
<th width="100">Code du march&eacute; </th>
<th width="80"><center>
...
<?php if($totalRows_liste_marche>0) { $nbm=1; $p1="j"; foreach($row_liste_marche1 as $row_liste_marche){ $id = $row_liste_marche['code_marche']; $Nombres_jours=0;  number_format($Nombres_jours, 0, ',', ' '); $duree_totale=0;
...
?>
 ...
 <?php if($p1!=$row_liste_marche['composante']) {?>
          ...
          <tr class="<?php echo ($i%2==0)?"odd":"even"; ?>">
            <td class=" ">&nbsp;</td>
            <td colspan="<?php echo count($TableauEtape)+8; ?>" class=" "><?php echo "<strong>".$row_liste_marche['code_marche'].": </strong> "; ?><?php echo $row_liste_marche['intitule']; ?></td>
            <td class=" " style="<?php echo (isset($exp[0]) && $exp[0]==$a)?"background-color: #EEEEEE;":""; ?>">&nbsp;</td>
          </tr>
          <tr class="<?php echo ($i%2==0)?"odd":"even"; ?>">
<td class=" "><div align="center"><?php echo "<strong>".$nbm."</strong> "; $nbm++; ?></div></td>

<td class=" ">&nbsp;</td>

<td class=" "><?php echo $row_liste_marche['categorie']; ?></td>

<td class=" "><?php echo $row_liste_marche['methode']; ?></td>

<td class=" " style="<?php echo (isset($exp[0]) && $exp[0]==$a)?"background-color: #EEEEEE;":""; ?>">&nbsp;</td>
<td nowrap="nowrap" class=" " style="<?php echo (isset($exp[0]) && $exp[0]==$a)?"background-color: #EEEEEE;":""; ?>"><?php  echo number_format($row_liste_marche["montant_usd"], 0, ',', ' ');  ?></td>

<td class=" " style="<?php echo (isset($exp[0]) && $exp[0]==$a)?"background-color: #EEEEEE;":""; ?>"><span class=" " style="<?php echo (isset($exp[0]) && $exp[0]==$a)?"background-color: #EEEEEE;":""; ?>">
...
// if(isset($amois[2])) {
         ...
         // foreach($as as $a){ if(!empty($a)) $ad[$a]=$b[$i]; $i++; }
		   //}  $TableauEtapeM[$row_liste_etape_modele["marche"]][$row_liste_etape_modele["groupe"]]
		    $i++; 
...
if( isset($amois[2]) && isset($TableauEtapeM[$amois[0]][$row_liste_marche["id_marche"]])) {
 echo  date("d/m/Y", strtotime('+'.$duree_totale.'days', strtotime($date_start)));
  }  else { if(isset($TableauEtapeM[$row_liste_marche["modele_marche"]][$amois[0]])) echo $TableauEtapeM[$row_liste_marche["modele_marche"]][$amois[0]] ;}?></td>
<?php } ?>
<td class=" " style="<?php echo (isset($exp[0]) && $exp[0]==$a)?"background-color: #EEEEEE;":""; ?>"><span class=" " style="<?php echo (isset($exp[0]) && $exp[0]==$a)?"background-color: #EEEEEE;":""; ?>">
  <?php  echo $duree_totale;  ?>
            ...
            <?php }  ?>
<tr class="even">
  <td colspan="<?php echo count($TableauEtape)+8; ?>"><div align="center" style="background-color:#CCCCCC; height: 2px;">&nbsp;</div></td>
</tr>
<?php  } } else { ?>
