...
<?php if(!isset($_GET["down"])){  ?>
<div class="widget-header no-print"> <h4 style="width: 49%"><i class="icon-reorder"></i><strong><?php echo $lib_nom_fich; ?></strong></h4><h4 align="right" style="width: 49%"><?php if(isset($_SESSION['clp_niveau']) && $_SESSION['clp_niveau']<3 && isset($cp) && !empty($cp) && $totalRows_act>0) {?><a href="<?php echo $editFormAction."&down=1&t=w&cp=$cp&annee=$annee"; ?>" title="Imprimer en Word"><img src="images/doc.jpg" width="23" height="22" alt="" /></a><?php }?>&nbsp;|&nbsp;<a href="<?php echo "s_programmation.php"; ?>" title="">R&eacute;tour</a></h4>
...
<?php if($totalRows_entete1>0){ do{ if($row_entete1["Field"]!="LKEY" && in_array($row_entete1["Field"],$entete1_array)){
...
<?php $i++; } while ($row_act1 = mysql_fetch_assoc($act1)); } ?>
</tbody></table>
<?php if(isset($_GET["down"])){  ?><tr style="border: none"><td style="border: none" colspan="3">&nbsp;</td></tr><?php } ?>
<?php }else echo "<h3 align='center'>Aucune colonne &agrave; afficher dans cette fiche ".$lib_nom_fich."!</h3>"; ?>

</td></tr>
<?php }
$i++; } while ($row_act = mysql_fetch_assoc($act));
 }else echo "<h3 align='center'>Aucune donn&eacute;e &agrave; afficher dans cette fiche ".$lib_nom_fich."!</h3>"; ?>
</tbody></table><?php }else echo "<h3 align='center'>Aucune colonne &agrave; afficher dans cette fiche ".$lib_nom_fich."!</h3>"; ?>
				   ...
				   <?php } else {?>
                  <tr>
                    <td colspan="21" nowrap="nowrap"><h3 align="center" class="Style5">Veuillez s&eacute;lectionnez une fiche !!! </h3></td>
                    </tr>
                  </table>
                  <?php } ?>
