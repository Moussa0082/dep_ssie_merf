...
if(isset($_POST["email"]))
{
  ...
  $lien = "https://ruche-nigelec.org/get_new_password.php";
  $query_clp = "SELECT * FROM ".$database_connect_prefix."personnel WHERE email=:email";
  try{
    $clp = $pdar_connexion->prepare($query_clp);
    $clp->execute(array(":email"=>$mail));
    $row_clp = $clp ->fetch();
  ...
  //$message = "<h3>Bonjour, ".strtoupper($row_clp['nom'])." ".$row_clp['prenom']."<br /><br />Vous avez demander &agrave; r&eacute;initialiser votre mot de passe d\'acc&egrave;s au Syst&egrave;me de suivi-&eacute;valuation RUCHE-PSAC. <a href=\'$lien?id=".$row_clp['id_personnel']."<>pass-c=".$row_clp['pass']."\' target=\'_blank\'>Cliquez ici</a>.<br /><br />Si vous n\'&ecirc;tes pas &agrave; l\'origine de cette demande, ignorer ce message tout simplement !</h3>";
}
