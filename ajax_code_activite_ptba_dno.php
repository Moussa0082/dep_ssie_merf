...
if(!empty($q))
{  */
    $a = array("ageroute_","aiph_","apromac_","cca_","ccc_","ic_","ucp_");
    if(!isset($_GET['annee'])) $annee=date("Y"); else $annee=intval($_GET['annee']);
  $where = ""; $resultat = "";
  mysql_select_db($database_pdar_connexion, $pdar_connexion);
  $query_liste_activite_1 = ""; $i=0;
  foreach($a as $b)
  {
    $query_liste_activite_1 .= "SELECT * FROM `".$b."ptba` WHERE projet='".$_SESSION["clp_projet"]."' and annee='$annee' ";
    if($i<count($a)-1) $query_liste_activite_1 .= " UNION "; $i++;
  }
