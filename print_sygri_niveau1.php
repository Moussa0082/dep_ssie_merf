...
header("content-disposition: attachment;filename=SYGRI_1er_Niveau_FIER.xls"); }
else if(isset($_GET["down"]) && isset($_GET["t"]) && $_GET["t"]=="w"){
header("Content-Type: application/vnd.ms-word");
header("Expires: 0");
header("Cache-Control: must-revalidate, post-check=0, pre-check=0");
header("content-disposition: attachment;filename=SYGRI_1er_Niveau_FIER.rtf"); }
...
<br />
<?php } ?>
<h4 align="center">Indicateurs SYGRI 1<sup>er</sup> Niveau du FIER</h4>

<table width="90%" border="1" align="center" cellspacing="0" bordercolor="#000">
  <tr bgcolor="#CCCCCC">
    <td nowrap="nowrap" colspan="4"><strong>Indicateurs par composante </strong></td>
  </tr>
  <?php if($totalRows_composante>0) {$o2=0; do { ?>
  <tr <?php if($o2%2==0) echo 'bgcolor="#FFF"'; $o2=$o2+1;?>>
    <td colspan="4" align="left" valign="middle"><span class="Style51"><?php echo $row_composante['code_sous_composante'].": ".$row_composante['intitule_sous_composante']; ?></span></td>
  </tr>
      <?php 
				    $id_cp=$row_composante['id_sous_composante'];
				    mysql_select_db($database_pdar_connexion, $pdar_connexion);
					$query_ind = "SELECT id_indicateur_sygri_niveau1_projet	,referentiel, cible_projet, id_groupe, nom_groupe, indicateur_sygri_niveau1, cible_rmp FROM indicateur_sygri1_projet, groupe_indicateur where id_groupe=groupe_indicateur and scomposante='$id_cp' order by code_groupe, ordre";
      ...
      <tr class="titrecorps2">
        ...
        <td align="center">Cible RPE </td>
        <td align="center">Cible RMP </td>
      </tr>
      <?php do { ?>
      <?php  if($p1!=$row_ind['id_groupe']) {?>
      <tr <?php  echo 'bgcolor="#D2E2B1"'; ?>>
        <td align="center" colspan="4"><div align="left">
          <?php  if($p1!=$row_ind['id_groupe']) {echo $row_ind['nom_groupe']; $i=0; }$p1=$row_ind['id_groupe']; ?>
        </div></td>
      </tr>
      <?php } ?>
      <tr <?php if($i%2==0) echo 'bgcolor="#F9F9F7"'; $i=$i+1;?>>
        <td width="70%"><div align="left" class="Style51"><?php echo $row_ind['indicateur_sygri_niveau1']; ?></div>
              <div align="left" class="Style51"> </div></td>
        <td ><div align="center"><span class="Style22"><?php if(isset($unite_ind_ref_array[$row_ind["referentiel"]])) echo " (".$unite_ind_ref_array[$row_ind["referentiel"]].")"; ?></span></div></td>
        <td><div align="center" class="Style51"><?php echo $row_ind['cible_projet'];?></div></td>
        <td><div align="center"><span class="Style51">
          <?php if(isset($row_ind['cible_rmp']) && $row_ind['cible_rmp']>0) echo $row_ind['cible_rmp']; else echo "-"; ?>
        </span></div></td>
      </tr>
      <?php } while ($row_ind = mysql_fetch_assoc($ind));  mysql_free_result($ind);?>
      <?php } ?>

  <?php } while ($row_composante = mysql_fetch_assoc($composante));  mysql_free_result($composante);  ?>
  <?php } else {?>
  <tr>
