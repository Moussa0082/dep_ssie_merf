...
if ((isset($_POST["MM_form"])) && ($_POST["MM_form"] == "form1"))
{
  ...
  if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "MM_insert")) {
   $personnel=$_SESSION['clp_id'];
    $insertSQL = sprintf("INSERT INTO referentiel_indicateur (intitule_ref_ind,composante,code_ref_ind, type_ref_ind,unite,mode_calcul,mode_suivi,beneficiaire,categorie, id_personnel) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, '$personnel')",
  ...
  $date=date("Y-m-d"); $personnel=$_SESSION['clp_id']; $c=$_POST['id'];
  	...
  	$insertSQL = sprintf("UPDATE referentiel_indicateur SET intitule_ref_ind=%s, composante=%s, code_ref_ind=%s, type_ref_ind=%s, unite=%s,mode_calcul=%s,mode_suivi=%s,beneficiaire=%s,categorie=%s, modifier_par='$personnel', modifier_le='$date' WHERE id_ref_ind='$c'",
                         GetSQLValueString($_POST['intitule_ref_ind'], "text"),
                         GetSQLValueString($_POST['composante'], "text"),
  					   GetSQLValueString($_POST['code_ref_ind'], "text"),
					   GetSQLValueString($_POST['type_ref_ind'], "int"),
...
<a href="objectif_general_cmr.php" title="Editer l'Objectif global" class="pull-right p11"><i class="icon-plus"> Objectif global </i></a>
<a href="objectif_developpement_cmr.php" title="Editer les objectifs de d&eacute;veloppement" class="pull-right p11"><i class="icon-plus"> Objectif de D&eacute;veloppement </i></a>
...
<div align="center">
              <table border="0" align="center" cellspacing="0" >
                <tr>
                  <td><div align="left" class="">Composante : &nbsp;</div></td>
                  <td valign="middle"><form name="form38" id="form38" method="get" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                    <select name="composante" onchange="form38.submit();" style="background-color: #FFFF00">
                      <option value="">-- Tous les composantes --</option>
                      <?php $p1="j"; if($totalRows_edit_composante>0){ do { $libelle = $row_edit_composante['intitule']; $libelle = substr($libelle,0,50)." ..."; ?>
                  <option value="<?php echo $row_edit_composante['code']; ?>" <?php if(isset($_GET["composante"]) && $row_edit_composante['code']==$_GET["composante"]) echo "selected='SELECTED'"; ?>><?php echo $row_edit_composante['code'].": ".$libelle;?></option>
                      <?php } while ($row_edit_composante = mysql_fetch_assoc($edit_composante));?>

<?php } else echo '<option>Aucune composante enregistre&eacute;</option>'; ?>
                    </select>
                  </form></td>
                </tr>
              </table>
...
<?php if($totalRows_liste_composante>0) { $i=0; do { if(isset($row_liste_composante['id_ref_ind']) && !empty($row_liste_composante['id_ref_ind']))$id = $row_liste_composante['id_ref_ind']; ?>
<tr class="<?php echo ($i%2==0)?"odd":"even"; ?>">
<td class=" "><?php echo $row_liste_composante['code']; ?></td>
<td class=" "><?php echo (!empty($row_liste_composante['intitule_ref_ind']))?$row_liste_composante['code_ref_ind'].": ".$row_liste_composante['intitule_ref_ind']:""; ?></td>
...
<td class=" "><?php  if(isset($row_liste_composante['mode_suivi']) &&  $row_liste_composante['mode_suivi']==1) echo "Activit&eacute;";  elseif(isset($row_liste_composante['mode_suivi']) &&  $row_liste_composante['mode_suivi']==2) echo "Etudes/Enqu&ecirc;tes"; else "ND"; ?></td>
...
?>
<!--<?php /* if(isset($_SESSION['clp_niveau']) && $_SESSION['clp_niveau']<2 && !empty($row_liste_composante['id_ref_ind'])) {?><a onclick="get_content('new_indicateur_ref.php','id=<?php echo $id; ?>','modal-body_add',this.title);" data-toggle="modal" href="#myModal_add" title="Indicateur <?php echo $row_liste_composante['intitule_ref_ind']; ?>" class="thickbox Add"  dir=""><img src="images/edit.png" width='20' height='20' alt='Modifier' /></a><?php } ?>
<?php if(isset($_SESSION['clp_niveau']) && $_SESSION['clp_niveau']<2 && !empty($row_liste_composante['id_ref_ind'])) {?><a href="<?php echo $_SERVER['PHP_SELF']."?id_sup=".$id; ?>" onclick="return confirm('Voulez-vous vraiment supprimer cet indicateur ?');" /><img src="images/delete.png" width="15" border="0"/></a><?php } */?>-->
</td>
</tr>
<?php } ?>
