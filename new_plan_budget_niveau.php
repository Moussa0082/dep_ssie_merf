...
$date=date("Y-m-d");
  ...
  $query_liste_activite = "SELECT * FROM ".$database_connect_prefix."plan_budget_config WHERE ".$_SESSION["clp_where"]." LIMIT 1";
  try{
...
try{
  ...
  $libelle = $code = array();
  if($totalRows_entete>0){ $libelle=explode(",",$row_entete["libelle"]); $code=explode(",",$row_entete["code_number"]); }
...
<script>
 ...
 var index= 0; var nbrch = <?php echo count($libelle); ?>;
	   ...
	   function add_input($container,num,lib_v,nodel,niv,code){
	    	       var $prototype=$(' <tr><td colspan="2"><label for="form_lib_'+index+'">'+'Niveau '+(index+1)+'</label> '+'<input placeholder="Libellé" required="required" type="text" id="form_lib_'+index+'" size="30" name="form[lib]['+index+']" value="'+((lib_v)?lib_v:'')+'" class="form-control" /><input type="hidden" id="form_niveau_'+index+'" size="30" name="form[niveau]['+index+']" value="'+(index+1)+'" /></td><td><label for="form_code_'+index+'">Taille code&nbsp;</label><input placeholder="Nombre de caractère du code" required="required" type="text" id="form_code_'+index+'" size="10" name="form[code]['+index+']" value="'+((code)?code:'')+'" class="form-control" />');
 ...
 add_input($container,"<?php echo $i; ?>","<?php echo $lib; ?>","nodel","<?php echo count($libelle)-1; ?>","<?php echo $code[$i]; ?>");
