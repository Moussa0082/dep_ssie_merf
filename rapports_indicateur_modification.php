...
<form id="form_rapport_dynamique" action="traitement_jquery\inserer_rapport_indicateur.php" method="POST">
  <input type="hidden" name="Code_Rapport" id="Code_Rapport" <?php echo 'value="'.base64_encode($row4["Code_Rapport"]).'"'; ?>>
   ...
   <div class="row" style="">
      <div class="col-md-4 col-lg-4">
        ...
        <select class="form-control" name="nom_rapport" id="nom_rapport">

     <?php foreach (FC_Rechercher_Code("SELECT * FROM v_indicateurs WHERE (code = '".$row4["Indicateur"]."')") as $row3) 
      {echo '<option value="'.$row3["code"].'">'.$row3["intitule"].'</option>';}?>
        
      <?php foreach (FC_Rechercher_Code("SELECT * FROM v_indicateurs WHERE (code NOT IN (SELECT Indicateur FROM t_rapport_indicateur))") as $row3) 
      {echo '<option value="'.$row3["code"].'">'.$row3["intitule"].'</option>';}
       ?>  
        </select>
      </div> 
  ...
  <?php 
        $ind=0;
      foreach (FC_Rechercher_Code('SELECT * FROM t_feuille_ligne WHERE Code_Feuille='.$row4["Code_Feuille"]) as $row7){
     if($row4["Attribut_Jointure_FP"]==$row7["Nom_Ligne"])
 ...
 ?>
      ...
      <div class="col-md-4 col-lg-4">
        <label style="float: left;">Reprouper par</label>
         <select name="input_regrouper_par" id="input_regrouper_par" class="form-control">
                <?php 
        $ind=0;
        if($row4["Group_By"]=="" OR $row4["Group_By"]==NULL)
        {echo '<option value=""></option>';
          foreach (FC_Rechercher_Code('SELECT * FROM t_feuille_ligne WHERE Code_Feuille='.$row4["Code_Feuille"]) as $row7){
          echo '<option value="'.$row7["Nom_Ligne"].'" class="option_champ_'.str_replace(" ", "", $row7["Nom_Ligne"]).'" id="">'.$row7["Nom_Ligne"].'</option>';}

          echo '<option value="ANNEE">ANNEE</option>';
        }
        else if($row4["Group_By"]=="ANNEE")
        {echo '<option value=""></option>';
          foreach (FC_Rechercher_Code('SELECT * FROM t_feuille_ligne WHERE Code_Feuille='.$row4["Code_Feuille"]) as $row7){
          echo '<option value="'.$row7["Nom_Ligne"].'" class="option_champ_'.str_replace(" ", "", $row7["Nom_Ligne"]).'" id="">'.$row7["Nom_Ligne"].'</option>';}
      ...
      foreach (FC_Rechercher_Code('SELECT * FROM t_feuille_ligne WHERE Code_Feuille='.$row4["Code_Feuille"]) as $row7){
     if($row4["Group_By"]==$row7["Nom_Ligne"])
     {echo '<option value="'.$row7["Nom_Ligne"].'" class="option_champ_'.str_replace(" ", "", $row7["Nom_Ligne"]).'" id="" selected>'.$row7["Nom_Ligne"].'</option>';}
   else {echo '<option value="'.$row7["Nom_Ligne"].'" class="option_champ_'.str_replace(" ", "", $row7["Nom_Ligne"]).'" id="">'.$row7["Nom_Ligne"].'</option>';}
      $ind++;
   }
...
document.getElementById("feuille_jointure").value="";
  if(document.getElementById("select_feuille").value!="")
    ...
    if(data!='')
    {$("#input_regrouper_par").html(data+'<option value="ANNEE">ANNEE</option>'); $("#input_valeur").html(data);  $("#attribut_jointure_fp").html(data); 
