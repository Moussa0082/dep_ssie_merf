...
$dir = './documents/'.$personnel.'/';
if(!is_dir($dir)) mkdir($dir);
$racine = $dir;
$dir = (isset($_GET['dir']) && !empty($_GET['dir']))?$_GET['dir']:'./documents/'.$personnel.'/';
if(strstr($dir, $racine)==false) $dir = $racine;
$b = './documents/'.$personnel.'/';
$vowels = array(".", "/", "%", "\\");
$tmp = explode($b,$dir); $j = 1;
if(isset($tmp[1]))
{
...
{ //Upload file
  if ((isset($_FILES['fichier']['name'])) && count($_FILES['fichier']['name'])>0) {
    ...
    $insertGoTo = $_SERVER['PHP_SELF'];
    //if($Result1) mysql_query_ruche("DOC".implode('|',$link), $pdar_connexion,1);
...
<div class="widget-header"> <h4><i class="icon-reorder"></i> Mes documents priv&eacute;s </h4></div>
...
<?php
  echo do_link("","","Ajout de fichier","Ajout de fichier","","./","pull-right p11","get_content('new_document.php','dir=$dir&racine=$racine&page=$nfile&parent=1','modal-body_add',this.title,'iframe','','',300);",1,"",$nfile);

  echo do_link("","","Cr&eacute;er un dossier","Cr&eacute;er un dossier","","./","pull-right p11","get_content('new_document.php','folder=1&dir=$dir&racine=$racine&page=$nfile&parent=1','modal-body_add',this.title,'iframe','','',300);",1,"",$nfile);
?>
<?php //} ?>
</div>
...
<?php //if(isset($_SESSION['clp_niveau']) && ($_SESSION['clp_niveau']==1)) { ?>
<?php
  echo do_link("","","Renommer le dossier","","edit","./","","get_content('new_document.php','&folder=1&dossier=$file&dir=$dir&racine=$racine&page=$nfile&parent=1','modal-body_add',this.title);",1,"margin:0px 5px;",$nfile);
...
<?php //if(isset($_SESSION['clp_niveau']) && ($_SESSION['clp_niveau']==1)) { ?>
<?php
  //echo do_link("","","Renommer le fichier","","edit","./","","get_content('new_document.php','dossier=$file&dir=$dir&racine=$racine&page=$nfile&parent=1','modal-body_add',this.title);",1,"margin:0px 5px;",$nfile);
