...
<?php endif ?>             
    <div id="sidebar" class="sidebar collapsed">
        ...
        <?php print isset($_SESSION["clp_projet_nom"])?$_SESSION["clp_projet_sigle"]:"MERF"; ?> <a href="../" ><img src="img/ruche.png"  width="50" height="34" alt="Retour au système"/> </a>
                    <span class="sidebar-close"><i class="fa fa-caret-left"></i></span>                       </h1>
                    <div id="seznamvrstev">                     </div>    
            </div>        
        </div>
    </div>
...
var map = new L.map('map', {
    //center: new L.LatLng(14.36, -14.47 ,95),
	   // center: new L.LatLng(15.90716, 7.1881 ,95),
	   // center: new L.LatLng(15.90716, 2.1881 ,95),
		 center: new L.LatLng(8, -0.008,7.4),
    zoom: 7.4,
    maxZoom: 15,
	
	   /* center: new L.LatLng(15.90716, 7.1881),
    zoom: 7.1,
    maxZoom: 15,*/
...
$sql_verif='SELECT * FROM information_schema.TABLES WHERE (TABLE_SCHEMA = \'c0ssi4627\') AND (TABLE_NAME = \''.$feuil['Nom_View'].'\')';
try{
...
if ($tbl_exist>0) {
  ...
  try{ 
    ...
    $totalRows_liste_chp=$liste_chp->rowCount();
      ...
      /*  $nom_tbl=$feuil['Nom_View'];
        $tbl_feuil[]=$nom_tbl;
        $tbl_feuil_lib[]=$feuil['intitule'];*/
		
		    $nom_tbl=$feuil['Nom_View'];
        $tbl_feuil[]=$nom_tbl;
		$nomlistef=$feuil['intitule'];
        $tbl_feuil_lib[]="$nomlistef";
        ...
        foreach ($op as $op) {
          $gps=($op['LG']).', '.($op['LT']);
		  		  ...
		  		  $libellec=$nomc=$opnomt=""; 
 /**/ if($totalRows_liste_chp>0){
	  foreach ($rows_liste_chp as $rows_liste_chp1) {  $libellec=$rows_liste_chp1['Libelle_Ligne']; $nomc=$rows_liste_chp1['Nom_Collone'];
		if(isset($op[$nomc]))  $opnomt.=str_replace($letters, $fruit, trim($libellec)).": <b>".str_replace($letters, $fruit, trim($op[$nomc]))."  </b><br/>";  //$nom_tbl=;
//echo $opnomt;
	  }
  }
	//$opnomt=""; 	  
  // exit;       //$data.='{ "type": "Feature","geometry": {"type": "Point","coordinates": ['.$gps.']},"properties":  { "text":"'.$feuil['Nom_Feuille'].'","labelPosition": ['.$gps.'],"radius":15,"name": "'.$feuil['Nom_Feuille'].'","description": "","icon": "js/theme/default/markers/2.png","couleur":"#3399FF" } },';
          $data.='{ "type": "Feature", "properties": { "id": 2165, "osm_id": "35346197", "z_order": 0, "'.$nom_tbl.'": "'.$feuil['intitule'].'", "building": null, "name": "'.$opnomt.'", "popup": "'.$feuil['fiche_carto'].'", "addr_house": null, "addr_hou_1": null, "addr_stree": null, "addr_city": null, "addr_state": null, "addr_postc": null, "addr_place": null, "addr_count": null, "addr_hamle": null, "addr_subur": null, "addr_subdi": null, "addr_distr": null, "addr_provi": null, "website": null, "contact_we": null, "phone": null, "contact_ph": null, "opening_ho": null, "brand": null, "OBJECTID": 1 }, "geometry": { "type": "Point", "coordinates": [ '.$gps.' ] } },';
        }
 ...
 function updateMarker(lat,lon) {
    ...
    //Add a marker to show where you clicked.
     theMarker = L.marker([lat,lon]).addTo(map);
              ...
              var mapycz = L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', {
                    maxZoom: 20,
...
for ($i=0; $i < count($tbl_file_zone_shp); $i++) { 
      ...
      echo 'var shpfile_'.$namereg.' = new L.Shapefile(\'../dep/map/leaflet.shapefile/'.$tbl_file_zone_shp[$i].'\', {
      onEachFeature: function(feature, layer) {
...
{map.setView([8, -0.008],7.4);}, 'home position',{ position: 'topright'});
 //center: new L.LatLng(7.60,-1.41,7),
   // center: new L.LatLng(14.36, -14.47,95),
