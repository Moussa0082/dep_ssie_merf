...
exit;
  ...
  $query_sup_cible_indicateur = "DELETE FROM ".$database_connect_prefix."cible_sygri_niveau1 WHERE indicateur_sygri='$id_ind'";
 ...
 //echo "je suis la";
  ...
  elseif(trim(strtolower($valind[$key]=="non"))) $valind[$key] = "1";
    ...
    $insertSQL = sprintf("INSERT INTO ".$database_connect_prefix."cible_sygri_niveau1  (indicateur_sygri, annee, valeur_cible, id_personnel, date_enregistrement) VALUES (%s, %s, %s, '$personnel', '$date')",
...
if(isset($_GET["id_ind"])){ $id_ind=$_GET['id_ind']; // $annee=$_GET['annee'];
   ...
   /* mysql_select_db($database_pdar_connexion, $pdar_connexion);
	...
	$query_indicateur_courant = "SELECT * FROM indicateur_sygri_cmr, resultat, indicateur_sygri where id_indicateur_sygri=indicateur_prd and id_resultat=resultat and id_indicateur=$id_ind";
	...
	$totalRows_indicateur_courant  = mysql_num_rows($indicateur_courant);*/
			...
			$query_cible_indicateur = "SELECT annee, valeur_cible FROM   ".$database_connect_prefix."cible_sygri_niveau1 where indicateur_sygri='$id_ind' and projet='".$_SESSION["clp_projet"]."'";
            ...
            if($totalRows_cible_indicateur>0){  do{ $cible_array[$row_cible_indicateur["annee"]]=$row_cible_indicateur["valeur_cible"]; }
			...
			$query_cible_tindicateur = "SELECT sum(valeur_cible) as cible_total FROM   ".$database_connect_prefix."cible_cmr_resultat where indicateur_sygri='$id_ind' group by zone";
