...
// query indicateur
mysql_select_db($database_pdar_connexion, $pdar_connexion);
$query_ind = "SELECT * FROM indicateur_objectif_global_cmr, indicateur_objectif_global WHERE ".$_SESSION["clp_where"]." and id_indicateur_objectif_global=indicateur_og  order by id_indicateur_objectif_global, code_cmr";
$ind  = mysql_query($query_ind , $pdar_connexion) or die(mysql_error());
$row_ind  = mysql_fetch_assoc($ind);
$totalRows_ind  = mysql_num_rows($ind);


if(isset($_GET["id_sup_indog"]) && $_SESSION['clp_niveau']==1) { $idiog=$_GET["id_sup_indog"];
mysql_select_db($database_pdar_connexion, $pdar_connexion);
$query_sup_ind = "DELETE FROM indicateur_objectif_global_cmr WHERE id_indicateur='$idiog'";
...
<div class="widget-header"> <h4><i class="icon-reorder"></i> Indicateurs d'impact </h4>
<?php
...
<table width="100%" border="0" align="center" cellspacing="1" class="table table-striped table-bordered table-responsive">
                      ...
                      <tr <?php if($i%2==0) echo 'bgcolor="#ECF0DF"'; $i=$i+1;?>>
                        <td><div align="left"><span class="Style27"><strong>Objectif G&eacute;n&eacute;ral: </strong></span> <span class="Style27"><?php echo $row_og['intitule_objectif_global']; ?></span></div><br />
                        <?php if(isset($_SESSION['clp_niveau']) && $_SESSION['clp_niveau']==1) {?>
<?php
echo do_link("","","Ajout d'indicateur &agrave; l'OG","Ajouter un indicateur","","./","pull-right","get_content('edit_indicateur_ogcmr.php','og=".$row_og['id_objectif_global']."','modal-body_add',this.title);",1,"margin:0px 5px;",$nfile); ?>
<!--<a onclick="get_content('edit_indicateur_ogcmr.php','og=<?php echo $row_og['id_objectif_global']; ?>','modal-body_add',this.title);" data-toggle="modal" href="#myModal_add" title="Ajout d'indicateur &agrave; l'OG" class="thickbox Add"  dir=""><span style="background-color:#ebebeb">&nbsp;<span class="Style5">Ajouter un indicateur</span>&nbsp;</span></a>  -->
                        <?php } ?></td>
                      </tr>
                      <tr>
      ...
      <?php if(isset($_SESSION['clp_niveau']) && $_SESSION['clp_niveau']==1) {?>
                      <td align="center">
<?php
echo do_link("","","Modifier indicateur : ".$row_ind['intitule_indicateur_cmr_og'],"","edit","./","","get_content('edit_indicateur_ogcmr.php','id=$id','modal-body_add',this.title);",1,"margin:0px 5px;",$nfile);

echo do_link("",$_SERVER['PHP_SELF']."?id_sup_indog=".$id,"Supprimer","","del","./","","return confirm('Voulez-vous vraiment supprimer cet indicateur ?');",0,"margin:0px 5px;",$nfile);
?>
<!--<a onclick="get_content('edit_indicateur_ogcmr.php','id=<?php echo $row_ind['id_indicateur']; ?>','modal-body_add',this.title);" data-toggle="modal" href="#myModal_add" title="Modifier" class="thickbox Add"  dir=""><img src='images/edit.png' width='20' height='20' alt='Modifier'></a>
<a href="<?php echo $_SERVER['PHP_SELF']."?id_sup_indog=".$row_ind['id_indicateur'].""?>" onclick="return confirm('Voulez-vous vraiment supprimer cet Indicateur ?');" /><img src="images/delete.png" width="15"/></a>-->
</td>
        <?php }?>
                          </tr>
