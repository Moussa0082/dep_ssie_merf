...
if(trim(strtolower($reference))=="n/a") $reference = -1;*/
  $insertSQL = sprintf("INSERT INTO indicateur_produit_cmr (indicateur_prd, referentiel, intitule_indicateur, unite_cmr, periodicite, cible_cmr,cible_rmp, responsable_collecte, cle, projet_prd, code_irprd, personnel, date_enregistrement) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, '$personnel', '$date')",
                       ...
                       GetSQLValueString((isset($_POST['referentiel'])?$_POST['referentiel']:0), "int"), 
	   				  ...
	   				  // GetSQLValueString($_POST['code_irprd'], "text"),
...
if(trim(strtolower($ciblermp))=="non") $ciblermp = 1;
if(trim(strtolower($ciblermp))=="n/a") $ciblermp = -1;
  $insertSQL = sprintf("UPDATE indicateur_produit_cmr SET  indicateur_prd=%s, referentiel=%s,  intitule_indicateur=%s, code_irprd=%s, unite_cmr=%s, periodicite=%s, cible_cmr=%s, cible_rmp=%s, responsable_collecte=%s, cle=%s, projet_prd=%s, code_irprd=%s, etat='Modifié', modifier_par='$personnel', modifier_le='$date' WHERE id_indicateur='$c'",
					   GetSQLValueString($_POST['indicateur_cl'], "int"),
                       GetSQLValueString((isset($_POST['referentiel'])?$_POST['referentiel']:0), "int"),
	   				   GetSQLValueString($_POST['indicateur'], "text"),
	   				   GetSQLValueString($_POST['code_irprd'], "text"),
...
$query_liste_composante = "SELECT 	indicateur_produit.*, indicateur_produit_cmr.*  FROM resultat, produit, indicateur_produit,indicateur_produit_cmr WHERE resultat.projet='".$_SESSION["clp_projet"]."' and id_produit=produit and id_resultat=effet and id_indicateur_produit=indicateur_prd order by code_resultat, code_produit, code_iprd, code_irprd";
try{
...
<div class="widget-header"> <h4><i class="icon-reorder"></i> Cadre de mesure de R&eacute;sultats (<span style="color:#FFFF00">Produits</span>) </h4>
    <?php if(isset($_SESSION['clp_niveau']) && $_SESSION['clp_niveau']<2){?>
	<a onclick="get_content('edit_indicateur_prdcmr.php','','modal-body_add',this.title);" data-toggle="modal" href="#myModal_add" data-backdrop="static" data-keyboard="false" title="Ajout d'indicateur de produit" class="btn btn-sm btn-warning pull-right p11" dir=""><i class="icon-plus"> Nouvel indicateur </i></a>
...
<th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" aria-label="Trier">Produit</th>
...
<th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" aria-label="Trier">Mode</th>
 ...
 <th class="" role="" tabindex="0" aria-controls="" aria-label="" <?php if($i==date("Y")) { ?>style="background-color:#FFCC33"   <?php } ?>><strong>
				      <?php
                       ...
                       <?php } ?> <th class="" role="" tabindex="0" aria-controls="" aria-label="" width="90">Cible DCP </th>
...
<td class=" "><?php if(isset($code_produit_array[$row_liste_composante['produit']])) echo $code_produit_array[$row_liste_composante['produit']]; ?></td>
<td class=" "><?php echo (!empty($row_liste_composante['intitule_indicateur']))?"<b>".$row_liste_composante['code_irprd']."</b>: ".$row_liste_composante['intitule_indicateur']:""; ?></td>
<td class=" "><span class="Style22"><?php if(isset($unite_ind_ref_array[$row_liste_composante["referentiel"]])) $unite = $unite_ind_ref_array[$row_liste_composante["referentiel"]]; else  $unite=""; echo $unite; ?>
</span></td>
<td class=" " align="center"><span class="Style22">
 ...
 <?php for($i=$_SESSION["annee_debut_projet"]; $i<=$_SESSION["annee_fin_projet"]; $i++){ ?>
    ...
    <?php } ?>
                       <td class=" " align="center"><a onclick="get_content('./edit_cible_indicateur.php','<?php echo "id_ind=".$row_liste_composante['id_indicateur']."&code_act=".$row_liste_composante['code_irprd']; ?>','modal-body_add',this.title,'iframe');" data-toggle="modal" href="#myModal_add" data-backdrop="static" data-keyboard="false" title="'<?php echo str_replace("'","\'",$row_liste_composante['intitule_indicateur']);?>'" class="thickbox" dir=""><?php echo (!empty($row_liste_composante['cible_cmr']))?"<b>".$row_liste_composante['cible_cmr']."</b>:":""; ?></a></td>
                   
<?php if(isset($_SESSION['clp_niveau']) && $_SESSION['clp_niveau']==1) { ?>
<td class=" " align="center">
<?php
echo do_link("","","Modifier Indicateur ".$row_liste_composante['intitule_indicateur'],"","edit","./","","get_content('edit_indicateur_prdcmr.php','id=$id','modal-body_add',this.title);",1,"margin:0px 5px;",$nfile);
