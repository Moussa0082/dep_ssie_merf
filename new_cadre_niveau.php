...
$date=date("Y-m-d");
  ...
  $query_liste_activite = "SELECT * FROM ".$database_connect_prefix."cadre_config WHERE projet='".$_SESSION["clp_projet"]."' LIMIT 1";
      	try{
...
<script>
 ...
 var index= 0; var nbrch = <?php echo count($libelle); ?>;
	   ...
	   if(index == 0){ //index++;
		  add_input($container,0,'Objectif Global',true);
          add_input($container,1,'Objectif de développement',true);
	   }
       <?php } ?>
       <?php if(count($libelle)<4){ ?>
	   ...
	   function add_input($container,num,lib_v,nodel,niv,t){
	    	       var $prototype=$(' <tr><td><label for="form_lib_'+index+'">'+'Niveau '+(index+1)+'&nbsp;</label>'+'<input placeholder="Libellé" required="required" type="text" id="form_lib_'+index+'" size="30" name="form[lib]['+index+']" value="'+((lib_v)?lib_v:'')+'" class="form-control" /><input type="hidden" id="form_niveau_'+index+'" size="30" name="form[niveau]['+index+']" value="'+(index+1)+'" /></td><td><label for="form_indic_'+index+'">Type&nbsp;</label><select name="form[indic]['+index+']" id="form_indic_'+index+'" required="required" class="form-control" ><option value="1" '+((t==1)?"SELECTED":"")+'>Produit</option><option value="2" '+((t==2)?"SELECTED":"")+'>Effet</option><option value="3" '+((t==3)?"SELECTED":"")+'>Impact</option></select>');
 ...
 add_input($container,"<?php echo $i; ?>","<?php echo $lib; ?>","<?php echo ($i>1)?true:''; ?>","<?php echo count($libelle)-1; ?>","<?php echo $type[$i]; ?>");
