...
$query_indicateur = "SELECT * FROM ptba, ".$database_connect_prefix."indicateur_tache where id_ptba=id_activite and annee=$annee and projet='".$_SESSION["clp_projet"]."' ORDER BY id_activite asc";
...
//semestre precedent
mysql_select_db($database_pdar_connexion, $pdar_connexion);
$query_cible_indicateur = "SELECT * FROM cible_indicateur_trimestre, indicateur_tache, ptba where id_ptba=id_activite and 	id_indicateur_tache=indicateur and annee=$annee and projet='".$_SESSION["clp_projet"]."' ";
...
$query_liste_ind_ptba = "SELECT indicateur_tache.unite, id_indicateur_tache, intitule_indicateur_tache, code_indicateur_ptba, indicateur_tache.id_activite FROM cible_indicateur_trimestre, indicateur_tache, ptba where id_ptba=id_activite and 	id_indicateur_tache=indicateur and annee=$annee and projet='".$_SESSION["clp_projet"]."' ";
...
$query_cible = "SELECT sum(cible) as valeur_cible, avg(cible) as valeur_cible_avg, id_ptba as id_activite, indicateur FROM cible_indicateur_trimestre, indicateur_tache, ptba where id_ptba=id_activite and 	id_indicateur_tache=indicateur and annee=$annee and projet='".$_SESSION["clp_projet"]."'  group by id_activite, indicateur";
//echo $query_cible;
//exit;
...
$query_valeur_suivi_ind = "SELECT sum(valeur_suivi) as valeur_reelle, avg(valeur_suivi) as valeur_reelle_avg, indicateur FROM suivi_indicateur_tache, indicateur_tache, ptba where id_ptba=id_activite and 	id_indicateur_tache=indicateur and annee=$annee and projet='".$_SESSION["clp_projet"]."' group by indicateur";
...
<?php } ?>
<div class="well well-sm"><strong>Situation des Indicateurs du PTBA <?php echo "$annee"; ?></strong></div>
...
<table width="<?php echo (!isset($_GET["down"]))?'':'100%'; ?>" border="<?php echo (!isset($_GET["down"]))?0:1; ?>" cellspacing="0" class="table table-striped table-bordered table-responsive">
            ...
            <tr bgcolor="<?php echo (!isset($_GET["down"]))?'':'#E4E4E4'; ?>">
              ...
              <th colspan="3" align="center"><center>TOTAL</center></th>
            </tr>
            <tr bgcolor="<?php echo (!isset($_GET["down"]))?'':'#E4E4E4'; ?>">
              <?php //for($j=1;$j<=4;$j++){ ?>
              ...
              <th align="center"><center>TAUX (%)</center></th>
            </tr>
            </thead>
...
<td align="center" width="70"><?php echo ($total>0 && $realval>0)?number_format(100*$realval/$total, 0, ',', ' ')." %":"-"; ?></td>

<td width="100" valign="middle" align="center"><?php  echo (!in_array($b['unite'],$array_indic))?$total:'-'; ?></td>
